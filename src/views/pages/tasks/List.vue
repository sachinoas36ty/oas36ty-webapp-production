<template>
  <div>
    ...
    <div class="block-header" style="margin-bottom: 12px; min-height: 60px">
      <div class="row d-flex" style="background: #fff; margin: 0">
        <div class="col-md-12 col-sm-12 col-xs-12">
          <div class="h-left clearfix">
            <div class="card">
              <div class="header d-flex justify-content-between">
                <div>
                  <h2 class="text-uppercase text-20-black">Tasks</h2>
                </div>
                <div class="d-flex justify-content-end">
                  <div class="d-inline-flex">
                    <b-button size="sm" variant="primary" class="mr-1" v-b-modal.modal-tasks>
        ...
        <b-col cols="12" md="6">
          <div class="d-flex align-items-center justify-content-end">
            <div class="btn-filter d-flex">
              ...
              <!-- <b-button variant="primary" @click="isCreateSidebarActive = true">
    ...
    <div style="margin-top: -71px">
      <b-tabs class="mb-2" v-model="activeTab">
        <b-tab active id="all">
          <template #title>
            ...
            <b-badge style="margin-left: 4px;" pill variant="dark">{{
              totalLength
            }}</b-badge>
          </template>
        </b-tab>
      ...
      <!-- <b-button  v-if="isAllchecked" size='sm' variant='primary'>
      ACTIONS
      <feather-icon icon="ChevronDownIcon" />
    </b-button> -->
      <div>
        <b-dropdown id="dropdown" text="ACTIONS" v-if="selectedCheckbox.length > 0" size="sm" variant="primary"
          style="margin-left: 1130px">
    ...
    <b-card no-body>
      ...
      <b-row v-if="isFilterActive == true" class="m-2">
        <b-col md="4">
          <b-form-group class="my-custom-select">
            <v-select v-model="filterStatus" :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'" label="Status"
              :options="statusOptions" placeholder="Select the Status" @input="filter()" />
          </b-form-group>
        </b-col>
        ...
        <b-col md="4">
          <b-form-group class="my-custom-select">
            <v-select v-model="FilterPriority" @input="filter()" :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'"
              label="star" :options="importance2.slice().reverse()" placeholder="Select the Priority"
              :readonly="!searchable">
        ...
        <b-col md="4">
          <b-form-group class="my-custom-select">
            <v-select v-model="filterUsers" label="name" :track-by="id" :options="paginated_users_to_assign"
              placeholder="Select the Assignee" @input="filter()" :searchable="true"
              :dropdown-should-open="dropdownShouldOpen">
              <!-- <li slot="list-footer" class="pagination">
      <b-button :disabled="!users_hasPrevPage" @click="offset -= limit">Prev</b-button>
      <b-button :disabled="!users_hasNextPage" @click="offset += limit">Next</b-button>
    </li> -->
        ...
        <b-col md="4" v-if="paginated_branch.length > 1" class="my-custom-select">
          <!-- <b-form-group> -->
          <v-select :options="paginated_branch" placeholder="Select the Branch Name" v-model="filterBranch"
            @input="filter()" @search="onSearch">
            <!-- <li slot="list-footer" class="pagination">
      <b-button :disabled="!hasPrevPage" @click="offset -= limit">Prev</b-button>
      <b-button :disabled="!hasNextPage" @click="offset += limit">Next</b-button>
    </li> -->
            <!-- <li slot="list-footer" class="pagination">
      <b-button :disabled="!hasPrevPage" @click="offset -= limit">Prev</b-button>
      <b-button :disabled="!hasNextPage" @click="offset += limit">Next</b-button>
        ...
        <b-col md="4">
          <b-form-group class="my-custom-select">
            <v-select :filterable="false" :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'"
              :options="paginated_companys" placeholder="Select the Client" v-model="filtercompany" @input="filter()"
              @search="onSearch">
              <!-- <li slot="list-footer" class="pagination">
      <b-button :disabled="!companys_hasPrevPage" @click="offset -= limit">Prev</b-button>
      <b-button :disabled="!companys_hasNextPage" @click="offset += limit">Next</b-button>
    </li> -->
        ...
        <b-col md="4">
          <b-form-group class="my-custom-select">
            <v-select v-model="filterCategory" :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'" :filterable="false"
              :options="paginated_category" placeholder="Select the Category" @input="filter()" @search="onSearch">
              <template #action>
                hello
              ...
              <!-- 
              <li slot="list-footer" class="pagination">
      <b-button :disabled="!category_hasPrevPage" @click="offset -= limit">Prev</b-button>
      <b-button :disabled="!category_hasNextPage" @click="offset += limit">Next</b-button>
    </li> -->
            </v-select>
          </b-form-group>
        </b-col>
        <b-col md="4">
          <b-form-group class="my-custom-select">
            <v-select v-model="filterContact" :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'" :filterable="false"
              :options="paginated_contactPerson" placeholder="Select the Contact Person" @input="filter()"
              @search="onSearch">
              <!-- <li slot="list-footer" class="pagination">
      <b-button :disabled="!contactPerson_hasPrevPage" @click="offset -= limit">Prev</b-button>
      <b-button :disabled="!contactPerson_hasNextPage" @click="offset += limit">Next</b-button>
    </li> -->
            </v-select>
          </b-form-group>
        </b-col>
        <b-col md="4">
          <b-form-group class="my-custom-select">
            <b-form-input id="tasks-search" v-model="filterSearch" @input="filter()"
              :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'" label="Search" placeholder="Search" />
          </b-form-group>
        </b-col>
      ...
      <b-table id="leads_table" :filterIncludedFields="['priority']" :show-empty="!isLoading && items.length === 0"
        :filter="FilterPriority != null ? FilterPriority.id : ''" :per-page="perPage" :current-page="currentPage"
        selectable responsive="sm" :items="items" :tbody-tr-class="rowClass" :fields="fields" class="mb-0"
          ...
          <div class="b-overlay-wrap position-relative">
            ...
            <b-card no-body class="ml-2 mt-2 mr-2 mb-0">
              <div class="d-flex justify-content-between" style="text-transform: uppercase; font-weight: 500">
                <span v-if="$router.currentRoute.name == 'tasks'"> Task Details </span>
                  ...
                  <b-container class="m-1">
                    <b-row>
                      ...
                      <b-col md="2" class="client-name234">
                        <span v-b-tooltip.hover.top="item.item.contact_person ? item.item.contact_person.name : ''
                          ">
                          {{
                      ...
                      <b-col md="2" class="client-name234">
                        <span v-b-tooltip.hover.top="item.item.category ? item.item.category.name : ''
                          ">
                          {{ item.item.category == null ? "-" : item.item.category.name }}
                        </span>
                      </b-col>
                    </b-row>
                    <b-row>
                      ...
                      <b-col md="2">
                        <span class="users-name" v-b-tooltip.hover.top="getAssigneeString(item.item.users)">
                          <!-- <span >{{ user.name }}</span>
<span v-if="item.item.users.length > 1">,</span> -->
   ...
   style="width: 100%!important;pointer-events:auto;height: 8rem; border: 1px solid white" :srcdoc="shortText(false)" frameborder="0"></iframe> -->
                  ...
                  <div style="line-height: 3; width: 32.333333333%">
                    ...
                    ">
                        <span class="ml-1">
                          ...
                          History
                          <b-badge v-if="getFollowUpData('history').length > 0">{{ getFollowUpData('history').length
                          }}</b-badge>
                        </span>
                      </div>
                      <div class="text-center mt-1" id="desc_history_followups"
                        ...
                        <!-- style="display: none; border: 1px solid #e0e0e0; line-height: 2.5"> -->
                        <div v-if="getFollowUpData('history').length > 0" v-for="data in getFollowUpData('history')"
                          :key="String(Number(data.id + 1))">
    ...
    margin-right: 1rem;">{{ getFollowUpDate(data) }}</small>
                    ...
                    ">
                        <span class="ml-1">
                          ...
                          Pending
                          <b-badge v-if="getFollowUpData('pending').length > 0">{{ getFollowUpData('pending').length
                          }}</b-badge>
                      ...
                      <div class="text-center mt-1" id="desc_pending_followups"
                        style="display: none; border: 1px solid #e0e0e0; line-height: 2.5">
                        <div v-if="getFollowUpData('pending').length > 0" v-for="data in getFollowUpData('pending')"
                          :key="String(Number(data.id + 1))">
                          <span v-if="data.activity_type.name === 'Visit'">
    ...
    margin-right: 1rem;">{{ getFollowUpDate(data) }}</small>
                    ...
                    ">
                        <span class="ml-1">
                          ...
                          <b-badge v-if="getFollowUpData('upcoming').length > 0">{{ getFollowUpData('upcoming').length
                          }}</b-badge>
                      ...
                      <div class="text-center mt-1" id="desc_upcoming_followups"
                        style="display: none; border: 1px solid #e0e0e0; line-height: 2.5">
                        <div v-if="getFollowUpData('upcoming').length > 0" v-for="data in getFollowUpData('upcoming')"
                          :key="String(Number(data.id + 1))">
                          <span v-if="data.activity_type.name === 'Visit'">
                            <input type="checkbox" v-model="data.checked" @change="markAsChecked(data, $event)" />
    ...
    margin-right: 1rem;">{{ getFollowUpDate(data) }}</small>
                    ...
                    ">
                          <span class="ml-1 d-flex justify-content-between">
                            ...
                            <div class="position-relative">
                              <div @click=" taskAttachmentDropdown()" style="margin-right: 5px">
                                ...
                                <div class="border-l">
                                  ...
                                  <div class="d-flex flex-column">
                                    <div
                                      ...
                                      <div>
                                        <input type="file" ref="file_task" multiple style="display: none"
                                          @change="uploadFile" />
                                        <b-dropdown size="sm" id="file_drop" class="" variant="primary">
                                          ...
                                          <b-dropdown-item v-b-modal.attachment-excel-modal><img class="mr-1"
                                              :src="excelIcon" width="25px" data-type="1" />Google
                                  ...
                                  <div class="p-1">
                                    ...
                                    <div v-else-if="fileLinks.filter(e => e.type === 'task').length">
                                      <div v-for="( file, index ) in  fileLinks.filter(e => e.type === 'task') "
                                        :key="index" class="d-flex justify-content-between">
                                        {{
                                          file.fileName.length > 35
                                          ? file.fileName.substr(0, 35)
                                          : file.fileName
                                        }}
                                    ...
                                    <div class="d-flex justify-content-end">
                                      <b-button size="sm" v-if="fileLinks.filter(e => e.type === 'task').length > 0"
                    ...
                    ">
                          <div
                            ...
                            <div class="ml-1 d-flex justify-content-between"
                              v-for="(  file, index  ) in   showFilesTask  " v-if="file.drive_url || file.attachment"
                              :key="index">
                              ...
                              <b-dropdown id="file_options" variant="link">
                                ...
                                <b-dropdown-item
                                  @click=" openInDrive(file.drive_url ? file.drive_url : file.attachment, file.original_name)">
                                  Download
                                </b-dropdown-item>
                    ...
                    ">
                          <span class="ml-1 d-flex justify-content-between">
                            ...
                            <div class="position-relative">
                              <div @click=" clientAttachmentDropdown($event)" style="margin-right: 5px">
                                <feather-icon icon="PlusIcon" size="18" />
                              </div>
                                ...
                                <div class="border-l">
                                  ...
                                  <div class="p-1">
                                    ...
                                    <div v-else-if="fileLinks.filter(e => e.type === 'company').length">
                                      <div v-for="( file, index ) in  fileLinks.filter(e => e.type === 'company') "
                                        :key="index" class="d-flex justify-content-between">
                                        {{
                                    ...
                                    <div class="d-flex justify-content-end">
                                      <b-button size="sm" v-if="fileLinks.filter(e => e.type === 'company').length > 0"
                                        variant="primary" @click="
                                          uploadAttach(
                                            item.item.id,
                                            item.item.company_id,
                    ...
                    ">
                          <div
                            ...
                            <div class="ml-1 d-flex justify-content-between" v-for="( file, index ) in  showFilesClient "
                              :key="index" v-if="file.drive_url || file.attachment">
                              <a :href="file.drive_url ? file.drive_url : file.attachment" target="_blank">{{
                                file.original_name.length > 38 ? file.original_name.substr(0, 38) : file.original_name
                              }}</a>
                              <b-dropdown id="file_options" class="" size="sm" variant="link">
                                <template #button-content>
                                  <span class="text-capitalize text-bolder"> ... </span>
                                </template>
                                <b-dropdown-item
                                  @click=" openInDrive(file.drive_url ? file.drive_url : file.attachment, file.original_name)">
                                  <!-- <a :href=" file.drive_url ? file.drive_url : file.attachment "> -->
                        ...
                        <div v-if="$store.state.tasks.isAttachment" class="b-overlay position-absolute"
                          style="inset: 0px;z-index: 10;/* display: none; */">
                          <div class="position-absolute bg-white rounded-sm"
                            style="inset: 0px; opacity: 0.75; backdrop-filter: blur(0px);"></div>
                          <div class="position-absolute"
              ...
              <b-tab v-if="setPrivileges.accounts === true">
                <template #title>
                  {{ "ACCOUNTS" }}
                  <b-badge v-if="allProposals.length != 0" style="margin-left: 0.5rem">{{
                    allProposals.length
                  }}</b-badge>
                </template>
                ...
                <b-table id="proposal_table" :show-empty="!isLoading && allProposals.length == 0"
                  ...
                  :current-page="currentPageProposalTable" selectable responsive :items="allProposals"
                  ...
                  <template #row-details="i">
                    <b-card no-body class="ml-2 mt-2 mr-2 mb-0">
                      <div v-if="i.item.title == 'proposal'" class="d-flex justify-content-between align-items-end"
                        ...
                        <div>
                          <!-- {{getStatusForProposalNumber(i.item)}}
                          {{getPercentProposal(i.item)}} -->
                          <!-- <div > -->
                          <b-button v-if="(Number(getPercentProposal(i.item)) < 100 && item.item.status_master.type !== 'closed')
                            " @click="() => {
      ...
      invoice.amount = null;
      invoice.amount = getAmountForProposals(i.item);
      ...
      // addAllProposalAmt();
      // $store.state.proposals.allProposals.forEach((element) => {
      //   if (element.id === i.item.id) {
      //    element.proposal_fees.forEach((e) //=> {
      //      invoice.amount = invoice.amount + //e.amount;
      //   });
      // }
      //  });
      getAmt();
    ...
    " variant="primary ml-1" size="sm">
                          ...
                          <!-- </div> -->
                          <b-button v-if="getPercentProposal(i.item) === 0" size="sm" @click="() => {
                              editIndex = i.index;
                              ...
                              openModal(
                                'proposal-edit-modal_' + item.item.id + '_' + i.item.id
                              );
                            }
                          ...
                          <b-button
                            v-if="$store.state.invoice.all.filter(e => e.proposal_id === i.item.id).length === 0"
                            size="sm" @click=" destroyProposal(i.item.id)" variant="primary ml-1">
                      ...
                      <div v-if="i.item.title == 'payment'" style="text-transform: uppercase; font-weight: 500"
                        ...
                        >
                          ...
                          <b-button size="sm" :disabled="loader_payment" variant="primary ml-1"
                            @click=" exportToPDFPayment(i)">
                            <span class="text-nowrap"> PDF</span>
                            <b-spinner class="ml-1" small v-if="loader_payment == true"></b-spinner>
                          </b-button>
                          <!-- <b-button
                          @click="destroyPayment(item.item.id)"
                          size="sm"
                          variant="primary ml-1"
                        >
                          <span class="text-nowrap"> Delete</span>
                      ...
                      <div v-if="i.item.title == 'invoice'" class="d-flex justify-content-between align-items-end"
                        ...
                        <div>
                          <b-button v-if="getPercent(i.item) < 100 && item.item.status_master.type !== 'closed'" @click="() => {
                              payment_arr = $store.state.invoice.all.filter(e => e.id === invoice.editId)
                              payment.amount = Math.round(getAmountForDueFunc(i.item))//Math.round(i.item.sub_total);
                              payment.mode = payment_modes[0];
                              payment.client = show_companys.filter(e => e.id === i.item.client_id)[0];
                              // payment.branch =paginated_branch[0];
                              $refs['payment-modal'].show();
                            }
                          ...
                          <b-button size="sm" @click="() => {
                            ...
                            " variant="primary ml-1">
                            <span class="text-nowrap"> Cancel Invoice</span>
                          </b-button>
                          <b-button v-if="getPercent(i.item) === 0" @click="() => {
                              invoiceTitle = 'edit';
                              ...
                              (invoice.id = i.item.invoice_number),
                                ...
                                (invoice.total_amt = i.item.sub_total);
                              invoice.sub_total = i.item.sub_total;
                              (invoice.pocket_expense = i.item.pocket_expenses),
                                ...
                                (invoice.total = i.item.total_amt);
                          ...
                          <b-button size="sm" :disabled="loader_invoice" variant="primary ml-1"
                            @click=" exportToPDFInvoice(i)">
                            <span class="text-nowrap"> PDF</span>
                            <b-spinner class="ml-1" small v-if="loader_invoice == true"></b-spinner>
                          </b-button>
                          <!-- <b-button variant="primary ml-1" @click="generateReport">
                          <span class="text-nowrap" >
                            PDF</span>
                        </b-button> -->

                          <b-button v-if="getPercent(i.item) === 0" size="sm" @click=" destroyInvoice(i.item.id)"
                            variant="primary ml-1">
                            <span class="text-nowrap"> Delete</span>
                          </b-button>
                        </div>
                      ...
                      <div v-if="i.item.title === 'Expense'">
                        <b-row>
                          ...
                          <b-col md="2">
                            <!-- {{ i.item }} -->
                            {{
                              i.item.users.length > 0
                              ? getAssigneeString(i.item.users)
                              : "-"
                            }}
                          </b-col>
                          <b-col md="2">
                            ...
                            <div class="row">
                              <div data-v-21ceb292="" class="col-md-8 padding-0">
                                ...
                                <div data-v-21ceb292="" class="receipt-inline clearfix">
                                  ...
                                  <div data-v-21ceb292="" class="receipt-value">
                                    {{ convertNumberToWords(Number(i.item.amount)) }}
                                  </div>
                                </div>
                              </div>
                          ...
                          <div class="col-md-12">
                            <div class="table-responsive">
                              <table class="table table-striped invoice-table" style="cursor: pointer">
                                ...
                                <tbody>
                                  <!-- {{ i.item }} -->
                                  <tr v-for="( p, index ) in  getMultipleAmountProposal(i.item.proposal) " :key="p.id">
                                    <td>{{ index + 1 }}</td>
                                    <td>{{ p.description }}</td>
                                    <td>{{ getPL(p.created_at, p.proposal_id) }}</td>
                                    <td>{{ getTotalAmt(getMultipleAmountProposal(i.item.proposal)) }}</td>
                                    <!---->
                                    <!---->
                                    <!---->
                                    <!---->
    ...
    > -->
                      ...
                      <div style="font-family: 'Times New Roman', Times, serif; display: none" id="InvoicepdfContent2"
                        v-if="i.item.title == 'invoice'">
                        <!-- <p>{{ i.item }}</p> -->
                        <!-- <div class=""> -->
                        <div style="margin-bottom: -3rem" class="row">
                          <div class="col-md-12">
                  ...
                  <template #cell(amount)="{ item }">
                    ...
                    <span v-else-if="item.title == 'payment'">
                      ...
                      {{ getTotalAmtInvoice(item.record_pay_invoice) }}
                    </span>
                    <span v-else-if="item.title == 'Expense'">
                      <!-- {{ getPaymentAmt(item.record_pay_invoice) }} -->
                      {{ item.expense }}
                ...
                <b-pagination :total-rows="allProposals.length" :per-page="perPageAccounts"
                  v-model="currentPageProposalTable" class="mb-0 mt-1 mt-sm-0" align="right" first-number last-number
                ...
                <div v-if="item.item.status_master.type != 'closed'" class="text-right">
                  <!-- <b-button v-if="allProposals.length > 0" class='mr-1' @click="() => {
                  invoiceTitle = 'add',
                  invoice.client = item.item.company,
                  invoice.gstNumber = '',
                  invoice.state = null,
                  ...
                  <b-button class="mr-1" variant="primary" @click="() => {
                      payment.title = 'add';
                      payment.reference_id = null
                      payment.notes = null
                      payment.date = currentDate
                      payment_arr = getArrInvoiceStatus()//$store.state.invoice.all
                      payment.payment.forEach((element) => {
                        element.tds = 0;
                        element.amount = 0;
                        element.amount23 = 0;
                  ...
                  <b-button variant="primary" @click="() => {
                    ...
                    " size="sm">Create Proposal</b-button>
                </div>
              </b-tab>
              <b-tab id="member-timesheet">
                <template #title>
                  {{ 'COST-SHEET' }}
                  <b-badge style="margin-left: 0.5rem" v-if="item.item.time_sheet.length > 0" variant="secondary">
    ...
    text-transform: none;" class="text-center mt-1" v-if="item.item.time_sheet.length === 0">There are no records to
        ...
        <!-- </template> -->
        <!-- <template #cell(selected_item)="rowSelected"> -->
        ...
        <template #cell(priority)="data">
          <div style="margin-left:1.7rem;"
            v-if="setPrivileges.onRowEdit === false || (data.item.status_master.type === 'closed' || data.item.status_master.type === 'completed')">
          ...
          <b-dropdown v-else v-ripple.400="'rgba(113, 102, 240, 0.15)'" variant="flat-dark">
            <template #button-content>
              <span>{{ !data.item.priority ? "-" : data.item.priority }}</span>
              <FeatherIcon :class="{ 'd-none': !data.item.priority }" :fill="SetColor(data.item.priority)"
                :style="{ color: SetColor(data.item.priority) }" icon="StarIcon" />
            ...
            <b-dropdown-item v-for=" impId  in  importance.slice().reverse() " :key="impId.id" @click="
              ...
              <span>
                {{ impId.id }}
                <feather-icon :fill="SetColor(impId.id)" :style="{ color: SetColor(impId.id) }"
                  :icon="impId.star" />{{
                    SetText(impId.id) }}
              </span>
            </b-dropdown-item>
        ...
        <template #cell(daysLeft)="data">
          ...
          <span style="margin-left:2rem;"
            v-if="(data.item.status_master.type === 'closed' || data.item.status_master.type === 'completed')" class="">
            <!-- {{ DaysLeft(data.item.created_at, data.item.due_date) + " days" }} -->
            -
          </span>
          <span style="margin-left:2rem;"
            v-if="setPrivileges.onRowEdit === false && data.item.status_master.type === 'open'" :class="{
                'text-red': DaysLeft(data.item.created_at, data.item.due_date) <= 0,
              }
              ">
            {{ DaysLeft(data.item.created_at, data.item.due_date) + " days" }}
          ...
          <span v-if="setPrivileges.onRowEdit === true && data.item.status_master.type === 'open'">
            <b-dropdown id="days_left_drop" menu-class="w-100" class="w-100" ref="dropdown" variant="flat-dark">
 ...
 <b-form-group id="open_datepicker2" >
        ...
        <template #cell(employees)="data">
          <span class="d-flex w-100 justify-content-center">
            ...
            <div class="mr-1"
              v-if="(data.item.status_master.type === 'closed' || data.item.status_master.type === 'completed')">
              -
            ...
            <div class="mr-1" v-if="setPrivileges.onRowEdit === false && data.item.status_master.type === 'open'">
              ...
              <span v-if="data.item.users.length == 0">{{ "-" }}</span>
              <b-avatar v-if="data.item.users.length > 0" v-for=" avatar  in  data.item.users " size="22px"
                :key="avatar.name" class="pull-up" :src="avatar.avatar" :title="avatar.name">
              </b-avatar>
            </div>

            <b-dropdown v-if="setPrivileges.onRowEdit === true && data.item.status_master.type === 'open'"
              v-ripple.400="'rgba(113, 102, 240, 0.15)'" variant="flat-dark" class="on-row-users">
              <template #button-content>
                <!-- {{user}} -->
                ...
                <b-badge v-b-tooltip.hover="getAssigneeString(data.item.users)"
                  v-if="data.item.users.length > 2 && getMoreUsers(data.item.users) > 0">+{{ getMoreUsers(data.item.users)
                  }}</b-badge>
                <!-- </b-avatar-group> -->
        ...
        <template #cell(status)="data">
          ...
          <b-badge
            v-if="DaysLeft(data.item.created_at, data.item.due_date) >= 0 && data.item.status_master.type === 'open'"
          ...
          <b-badge
            v-else-if="DaysLeft(data.item.created_at, data.item.due_date) < 0 && data.item.status_master.type === 'open'"
            :variant="'warning'
              ">
            <!-- <span v-if="DaysLeft(data.item.created_at, data.item.due_date) <= 0">
        ...
        <!--<template #cell(status)="data">
          <b-button v-ripple.400="'rgba(255, 255, 255, 0.15)'" :variant="buttonVariant(data.item.status)" size="sm">
            {{ data.item.status }}
          </b-button>
        </template>
          ...
          <b-row>
            <b-col md="6">
              <b-form-group label="Proposal Date">
                ...
                <VueDatePicker format="DD-MM-YYYY" :minDate="currentDate"
            ...
            <b-col class="d-flex flex-column" md="4">
              <ValidationProvider #default="validationContext" name="amount" vid="amount" rules="required">
                <b-form-group label="Amount *" class="my-custom-select *">
                  <b-form-input type="number" step="any" id="amount"  
                    v-model="amount.amount" placeholder="Enter Amount" />
          ...
          <b-row>
            <b-col class="py-1 d-flex justify-content-end" md="12">
              <div>
                ...
                <b-button v-if="proposalFields.length > 0" :disabled="$store.state.proposals.isLoading" size="sm"
                  class="mr-1" variant="primary" @click="saveAsTemplate()">
          ...
          <b-row>
              ...
              <b-form-group v-if="invoiceTitle == 'add'">
                ...
                <ValidationProvider #default="validationContext" name="Client" vid="Client" rules="required">
                  <v-select v-on:input="autofillClient" label="name" v-model="invoice.client"
                    placeholder="Search Client" class="select-size-md" :options="show_companys">
                    <template #list-header>
                      <div class="ml-2 mb-1 mt-1" v-b-modal.modal-clients>
            ...
            <b-col md="4">
              <b-form-group label="Client GST Number">
                <b-form-input @input="pan" v-bind:class="(invoice.gstNumber.length > 0 && invoice.gstNumber.length !== 15) ||
                    (invoice.gstNumber.length === 15 && gsterr === true)
                    ? 'is-invalid'
                    : ''
                  " v-model="invoice.gstNumber" placeholder="Client GST Number">
                </b-form-input>
                <span v-if="(invoice.gstNumber.length > 0 && invoice.gstNumber.length !== 15) ||
                  (invoice.gstNumber.length === 15 && gsterr === true)
            ...
            <b-col md="4">
              <b-form-group>
                ...
                <v-select @input="getStateCode" label="name" v-model="invoice.state" placeholder="Search State"
                  class="select-size-md" :options="allStates">
                </v-select>
              </b-form-group>
            </b-col>
            <!-- </b-card-body> -->
          ...
          <b-row>
            <b-col md="6">
              <b-form-group label="Billing Address">
                <b-form-input v-model="invoice.billing_address" placeholder="Billing Address" />
                ...
                <table border="0" id="invoice-table" class="table">
                  ...
                  <tbody style="line-height: 4">
                    <tr>
                      ...
                      <td style="width: 15%">
                        <b-form-input @input="getAmt" class="text-right" placeholder="Amount" style="
                            border-bottom-color: #1f91f3 !important;
                            border-top: none;
                            background-color: #f6f6f6 !important;
                            border-left: none;
                      ...
                      <td style="width: 10%">
                        <b-form-input @input="getAmt" class="text-right" placeholder="Discount" style="
                            border-bottom-color: #1f91f3 !important;
                            border-top: none;
                            background-color: #f6f6f6 !important;
                            border-left: none;
                      ...
                      <td style="width: 15%">
                        <b-form-input class="text-right" placeholder="Taxable Amount" style="
                            border-bottom-color: #1f91f3 !important;
                            border-top: none;
                            background-color: #f6f6f6 !important;
                            border-left: none;
                      ...
                      <td style="width: 10%">
                        <b-form-input @input="getAmt" class="text-right" placeholder="IGST" style="
                            border-bottom-color: #1f91f3 !important;
                            border-top: none;
                            background-color: #f6f6f6 !important;
                            border-left: none;
                      ...
                      <td style="width: 10%">
                        <b-form-input readonly class="text-right" placeholder="IGST" style="
                            border-bottom-color: #1f91f3 !important;
                            border-top: none;
                            background-color: #f6f6f6 !important;
                            border-left: none;
                    ...
                    <tr>
                      ...
                      <td style="width: 15%"></td>
                      <td style="width: 10%">
                        <b-form-input @input="getAmt" class="text-right" placeholder="SGST" style="
                            border-bottom-color: #1f91f3 !important;
                            border-top: none;
                            background-color: #f6f6f6 !important;
                            border-left: none;
                      ...
                      <td style="width: 10%">
                        <b-form-input readonly class="text-right" placeholder="SGST" style="
                            border-bottom-color: #1f91f3 !important;
                            border-top: none;
                            background-color: #f6f6f6 !important;
                            border-left: none;
                    ...
                    <tr>
                      ...
                      <td style="width: 10%">
                        <b-form-input @input="getAmt" class="text-right" placeholder="CGST" style="
                            ...
                            border-left: none;
                            border-right: none;
                          " type="number" step="0.01" v-model="invoice.cgst"></b-form-input>
                      </td>
                      <td style="width: 10%">
                        <b-form-input readonly class="text-right" placeholder="CGST" style="
                            border-bottom-color: #1f91f3 !important;
                            border-top: none;
                            background-color: #f6f6f6 !important;
                            border-left: none;
                    ...
                    <tr>
                      ...
                      <td style="width: 10%">
                        <b-form-input readonly class="text-right" placeholder="UTGST" style="
                            border-bottom-color: #1f91f3 !important;
                            border-top: none;
                            background-color: #f6f6f6 !important;
                            border-left: none;
                    ...
                    <tr>
                      ...
                      <td class="text-right">
                        <b-form-input @input="getAmt" style="
                            ...
                            border-left: none;
                            border-right: none;
                          " class="text-right" type="number" step="0.01" v-model="invoice.pocket_expense"
                    ...
                    <tr>
                      ...
                      <td class="text-right">
                        <b-form-input v-model="invoice.expense_details" style="
                            ...
                            border-left: none;
                            border-right: none;
                          " class="text-right" placeholder="out of pocket expense details" />
      ...
      <validation-observer #default="{ handleSubmit }" ref="refFormObserver">
        <b-form @submit.prevent=" handleSubmit(editProposal)">
          <b-row>
            ...
            <b-col md="6">
              <ValidationProvider #default="validationContext" name="concerned person" vid="person" rules="required">
            ...
            <b-col class="px-1" md="12">
              <div class="actions-proposal my-1 p-2 d-flex justify-content-center bg-light">
                ...
                <b-button @click="() => {
                  ...
                  " v-show="proposalFields.length === 0" class="ml-1" variant="primary" size="sm">
          ...
          <b-row v-for="( amount, index ) in  amountField " :key="index">
            <b-col md="8">
              <b-form-group label="Description">
                <b-form-input v-model="amount.description" placeholder="Enter Description" />
              </b-form-group>
            </b-col>
            <b-col class="d-flex flex-column" md="4">
              <ValidationProvider #default="validationContext" name="amount" vid="amount" rules="required">
                <b-form-group label="Amount *">
                  <b-form-input type="number" step="any" v-model="amount.amount" placeholder="Enter Amount" />
              ...
              <div class="text-right">
                <b-button v-show="amountField.length > 1" size="sm" @click=" removeAmount(index)"
                  variant="outline-primary">
                  Remove
                </b-button>
                <b-button v-show="index === amountField.length - 1" class="ml-1" size="sm" @click=" addAmount()"
                  variant="primary">
                  Add more
                </b-button>
              </div>
          ...
          <b-row>
            <b-col md="12">
              <b-form-group label="Internal Notes (Not visible for clients)">
                <b-form-input v-model="pnotes" placeholder="Enter Notes"> </b-form-input>
              </b-form-group>
            </b-col>
          </b-row>
          <b-row>
      ...
      <b-row>
        ...
        <b-col md="6">
          <b-form-group label="Next Number">
            <b-form-input v-model="lastInvoiceId" placeholder="Next Number"></b-form-input>
          </b-form-group>
        </b-col>
      </b-row>
      <!-- <b-row> -->
        ...
        <b-form @submit.prevent="
          ...
          <b-row>
            <b-col md="4">
              <b-form-group label="Client Name *" class="my-custom-select">
                <ValidationProvider #default="validationContext" name="Client" vid="Client" rules="required">
                  <v-select v-model="payment.client" placeholder="Search Client" :options="show_companys"
                    label="name">
                  </v-select>
                  <b-form-invalid-feedback class="d-block">
                    {{ validationContext.errors[0] }}
            ...
            <b-col md="4">
              <b-form-group label="Payment Mode *" class="my-custom-select">
                <ValidationProvider #default="validationContext" name="Client" vid="Client" rules="required">
                  <v-select v-model="payment.mode" placeholder="Payment Mode " :options="payment_modes" label="name">
                  </v-select>
                  <b-form-invalid-feedback class="d-block">
                    {{ validationContext.errors[0] }}
                  </b-form-invalid-feedback>
            ...
            <b-col md="4">
              <b-form-group label="Payment Amount *" class="my-custom-select">
                <ValidationProvider  #default="validationContext" name="payment amount" vid="Client" rules="required">
                  <b-form-input @input="checkValid" v-model.number="payment.amount" type="number" step="any"></b-form-input>
                  <b-form-invalid-feedback class="d-block">
                    {{ validationContext.errors.length > 0 ? validationContext.errors[0] : errors.paymentAmount }}
                  </b-form-invalid-feedback>
                </ValidationProvider>
              </b-form-group>
            </b-col>
          ...
          <b-row>
            ...
            <b-col md="4">
              <b-form-group label="Payment Date" class="my-custom-select">
                <VueDatePicker format="DD-MM-YYYY" :minDate="currentDate"
                  style="border: 1px solid #d8d6de; border-radius: 0.357rem" v-model="payment.date" />
              </b-form-group>
            </b-col>
            <b-col md="4">
...
" class="">
                      ...
                      <td aria-colindex="4" role="cell" class="">
                        {{ Math.round(invoice.sub_total) }}
                      </td>
                      <td aria-colindex="5" role="cell" class="">
                        {{ Math.round(getAmountForDueFunc(invoice)) }}
                      </td>
                      <td v-if="payment.tax">
                        <b-form-input :disabled="seteditPayment(invoice.id) === 0" @input=" setTds(invoice.id, $event)"
                          :value="setedittds(invoice.id)" type="number" step="any" class="hide form-control"
                          :id="String(invoice.id)"></b-form-input>
                      </td>
                      <td aria-colindex="6" role="cell" class="">
                        <div class="form-group m-0">
                          ...
                          <div class="form-line">
                            <b-form-input  @input=" setPayment(invoice.id, $event)" type="number" step="any"
          ...
          <div class="mt-1 text-right">
            ...
            <b-button type="submit" variant="primary" size="sm" :disabled="$store.state.recordPayment.sidebarLoading">
              {{ payment.title == "add" ? "Add" : "Update" }}
              <b-spinner small class="ml-1" v-if="$store.state.recordPayment.sidebarLoading" />
    ...
    <b-modal no-close-on-backdrop id="subtask-modal" ref="subtask-modal" size="lg" title="update checklist" hide-footer>
      ...
      <b-row>
        <b-col md="12">
          <b-form-group class="text-truncate" label="Enter sub-task Steps and assign @someone">
            <!-- </b-form-input> -->

            <Mentionable style="width: 100%" :keys="['@', '#']" :items="users_to_assign" offset="6" insert-space
              @open="onOpen" @apply="onApply">
              <b-form-input id="subtask_todo" v-model="todo_subtask" placeholder="" />
              <!-- <div></div> -->
    ...
    <edit-client :is-edit-sidebar-active.sync="ClientisEditSidebarActive" :edit-item.sync="editItemClient"
      @refetch-data="() => {
        ...
        <b-row>
          ...
          <b-col md="3">
            <b-form-group label="To">
              <v-select multiple v-model="composeData.to" :options="emails_contact"
                placeholder="Select To"></v-select>
            </b-form-group>
          </b-col>
          <b-col md="3">
            <b-form-group label="Cc">
              <v-select multiple v-model="composeData.cc" :options="emails_contact"
                placeholder="Select Cc"></v-select>
            </b-form-group>
          </b-col>
          <b-col md="3">
            <b-form-group label="Bcc">
              <v-select multiple v-model="composeData.bcc" :options="emails_contact"
            ...
            <div class="border-l">
              <div class="d-flex flex-column">
                <div class="d-flex justify-content-between align-items-center p-1 border-bottom-light bg-light">
                  <div>Attachment(s)</div>
                  <div>
                    <input type="file" multiple id="file_attachment" @change="onFileChange2" style="display: none" />
...
<script>
import DueDateModalVue from "./DueDateModal.vue";
...
import SearchMessages from "../leads/SearchMessages.vue";
...
import EditSidebar from "../leads/EditSidebar.vue";
...
import CreateLead from "../leads/CreateLead.vue";
...
export default {
  ...
  data() {
    ...
    return {
      ...
      pfooterdesc: `${JSON.parse(localStorage.getItem("userData")).name}<br/>For, ${JSON.parse(localStorage.getItem("userData")).tenant.organization.name
  ...
  computed: {
    ...
    expense_users() {
      return this.all_expense_users.slice(
        this.offset_users,
    ...
    allProposals() {
      let payment = this.$store.state.recordPayment.all.sort(
        (a, b) => new Date(b.created_at) - new Date(a.created_at)
      );
      let d1 = this.$store.state.invoice.all.sort(
        (a, b) => new Date(b.created_at) - new Date(a.created_at)
      );
      let d2 = this.$store.state.proposals.allProposals.sort(
        (a, b) => new Date(b.created_at) - new Date(a.created_at)
      );
    ...
    totalCount() {
      if (this.$router.currentRoute.name == "tasks") {
        ...
        var count = 0;
        if (this.activeTab === 1) {
          count = this.items.filter((e) => e.priority === 5 || e.priority === 4).length;
          ...
          count = this.items.filter((e) => this.DaysLeft(e.created_at, e.due_date) < 0).length;
      ...
      if (this.setPrivileges.viewAll === true) {
        ...
        if (this.$router.currentRoute.name == "tasks") {
          ...
          if (this.activeTab === 0) {
            ...
            //console.log(today)
            //taking next 7 days as week
            items = items.filter(
              (e) =>
                this.DaysLeft(e.created_at, e.due_date) <= 6 &&
                this.DaysLeft(e.created_at, e.due_date) > 0
            );
            // taking as a this week values(incomplete)
            // items=items.filter(e => this.DaysLeft(e.created_at, e.due_date)+today-7<0 && this.DaysLeft(e.created_at, e.due_date)+today-7<=3  )
            //  items=items.filter(e =>console.log(this.DaysLeft(e.created_at, e.due_date)+today-7))
          }
        } else {
        ...
        var items = [];
        if (this.$router.currentRoute.name == "tasks") {
          this.$store.state.leads.all.filter(e => e.type === 'task').forEach(a => {
    ...
    category_hasNextPage() {
      const nextOffset = this.offset + this.limit;
      return Boolean(this.category.slice(nextOffset, this.limit + nextOffset).length);
    },
    category_hasPrevPage() {
      const prevOffset = this.offset - this.limit;
      return Boolean(this.category.slice(prevOffset, this.limit + prevOffset).length);
    },
    branches() {
      var data = [];
    ...
    hasNextPage() {
      const nextOffset = this.offset + this.limit;
      return Boolean(this.branches.slice(nextOffset, this.limit + nextOffset).length);
    },
    hasPrevPage() {
      const prevOffset = this.offset - this.limit;
      return Boolean(this.branches.slice(prevOffset, this.limit + prevOffset).length);
    },
    users_to_assign() {
      ...
      return data.filter((country) =>
        country.name.toLocaleLowerCase().includes(this.search.toLocaleLowerCase())
      );
    },
    paginated_users_to_assign() {
      let data = this.users_to_assign
      // if(this.search || this.filterUsers){
      //   this.users_to_assign
      // }
    ...
    users_hasNextPage() {
      const nextOffset = this.offset + this.limit;
      return Boolean(this.users.slice(nextOffset, this.limit + nextOffset).length);
    },
    users_hasPrevPage() {
      const prevOffset = this.offset - this.limit;
      return Boolean(this.users.slice(prevOffset, this.limit + prevOffset).length);
    },
    contactPerson() {
      // return this.$store.state.contactPeople.allForLeads
      var data = [];
  ...
  beforeUpdate() {
    ...
    if (this.$router.currentRoute.params.overdueTask_status === true) {
      this.activeTab = 2
      // alert(this.activeTab)
    ...
    if (this.$router.currentRoute.params.taskDueThisWeek_status === true) {
      this.activeTab = 5
      // alert(this.activeTab)
  ...
  methods: {
      ...
      for (let index = 0; index < this.payment.payment.length; index++) {
        ...
        if(this.payment.payment[index].amount > 0){
          ...
          if(this.payment.payment[index].amount !== this.payment.amount){
            ...
            this.errors.paymentAmount = 'The total amount should be equal to payment amount';
            return;
          }else{
            this.errors.paymentAmount = ''
          }
        }
      }
    },
    ...
    for (let index = 0; index < this.items.length; index++) {
      // const element = array[index];
      arr[index] = [this.items[index].priority,this.items[index].id,this.items[index].subject,this.items[index].client ? this.items[index].client.name:this.items[index].contact_person.name,this.items[index].subject,this.formatDate(this.items[index].created_at),this.DaysLeft(this.items[index].created_at,this.items[index].due_date),this.items[index].status_master.type  ]
    }
   ...
   const data = [
    ...
    getMultipleAmountProposal(data){
      if(data.length === 1){
        return data;
      }else{
        let arr= []
    ...
    getPercent(data) {
      ...
      if (this.getStatusForInvoice(data) === 'not paid') {

        return 0
      } else {
        return Number(this.getStatusForInvoice(data).split('%')[0])
      }
    },
    ...
    getTotalCost(timeSheetData) {
      ...
      timeSheetData.forEach(element => {
        if (this.$store.state.users.all.find(e => e.id ===
    ...
    getAmountForProposals(data) {
      ...
      let percent = this.getStatusForProposalNumberShowPercent(data)
      console.log("this is percent", percent);
      let totalFees = 0;
      data.proposal_fees.forEach((i) => {
        totalFees = totalFees + i.amount;
      })
      let invoiceAmount = 0
      let amtinvoice = this.$store.state.invoice.all.filter(e => e.proposal_id === data.id)
      amtinvoice.forEach(element => {
        invoiceAmount = invoiceAmount + element.amount
      });
      amount = totalFees - invoiceAmount//((percent / 100) * totalFees);
      console.log("this is after the amount", totalFees)
      // alert(amount);
      return amount;
    },
    getAmountForDueFunc(data) {
      ...
      let arr = 0;
      this.$store.state.recordPayment.all.forEach((e) => {
        e.record_pay_invoice.forEach((eg) => {
          if (eg.invoice_id === data.id) {
            arr = arr + eg.paid_amount;
          }
        })
      })

      amount = data.sub_total - arr//((percent / 100) * data.sub_total);
      return amount;
    },
    select() {
    ...
    addExpense() {
      ...
      this.$store.dispatch("expenses/store", data).then(() => {
        ...
        this.expense.users = this.all_expense_users.filter(
          (e) => e.email === JSON.parse(localStorage.getItem("userData")).email
        );
    ...
    removeFileS3Task(url, index) {
      ...
      this.$store.state.tasks.s3_url.splice(this.$store.state.tasks.s3_url[index], 1);
      this.$store
    ...
    async totalBadgeCounts() {
      // let data=this.$store.state.tasks.all.filter(e => e.type === 'task');
      let data = [];
      ...
      if (this.setPrivileges.viewAll === true) {
        ...
        data = this.$store.state.leads.all.filter(e => e.type === 'task')//.filter(e => e.type === 'lead');
      } else {
        this.$store.state.leads.all.filter(e => e.type === 'task').forEach(a => {
          if (a.users.filter(e => e.email === JSON.parse(localStorage.getItem('userData')).email).length > 0) {
            data.push(a)
    ...
    getStatusForInvoice(data) {
      ...
      console.log('there is totsl: ', total)
      percent = (Math.round(status) / Math.round(total)) * 100;
      console.log(percent);
    ...
    getArrInvoiceStatus() {
      ...
      this.$store.state.invoice.all.forEach((i) => {
        // if (i.id === data.id) {
        // alert()
        // total = i.total_amt;
        // statusArr.forEach(element => {
        // alert()
        let status = Number(this.getStatusForInvoice(i).split('%')[0])
        //   percent = (element / total) * 100;
        if (status < 100 || this.getStatusForInvoice(i) === 'not paid') {
      ...
      console.log(percent);
      if (arr.length) {
        return arr//percent.toFixed(2) + "% paid";
      }
    },
    ...
    getStatusForProposal(data) {
      ...
      console.log(data);
      if (state) {
        return percent.toFixed(2) + "% invoiced";
      } else {
        return 'not invoiced'
      }
    },
    ...
    getStatusForProposalNumberShowPercent(data) {
      // console.log("status");
      // return this.getStatusForProposal
      console.log("status");
      var percent = 0;
      var total = 0;
      var status = 0;
    ...
    recordPayment() {
      ...
      //   }
      // });
      let tasks = this.$store.state.leads.all

      const data = {
        task_id: this.taskId,
        client_id: this.payment.client.id,
        branch_id: tasks.filter(e => e.id === this.taskId)[0].branch_id,
        pay_date: this.payment.date,
        payment_mode: this.payment.mode.name,
        amount: this.payment.amount,
        reference_id: this.payment.reference_id,
    ...
    setTds(index, e) {
      ...
      this.payment.payment.forEach((element) => {
        if (element.invoice_id === index) {
          ...
          if (element.tds != 0 && element.amount != 0) {
            element.amount23 = element.amount - element.tds;
          }
        }
      });
    ...
    getLargestId() {
      ...
      //   // this.invoice.prefix = prefix
      //   // let updated_prefix = prefix.substring(0,prefix.length - 1)
      //   this.invoice.prefix = prefix//updated_prefix;
      //   // this.invoice.id = u//pdated_prefix+this.invoice.id  //+ Number(prefix.split('/')[2])+1 //+ "/" + this.invoice.id;
      ...
      if (this.$router.currentRoute.name == "leads") {
        this.$store.dispatch("leads/inline_update", { data }).then(() => {
          this.$store.dispatch("leads/fetch").then(() => {
            // if (this.$store.state.leads.isLoadingLeads == true) {
            // this.$refs.snackbar[t](this.successMessage)
            this.$toast.success("Priority Changed Successfully!", {
              position: "bottom-left",
              timeout: 5000,
              closeOnClick: true,
              pauseOnFocusLoss: true,
...
// @import '@/assets/scss/vendors/bootstrap-vue/index' ;</style>
