<template>
  ...
  <b-card id="compose-mail" class="compose-reply" modal-class="modal-sticky"
    ...
    <validation-observer #default="{ handleSubmit }" ref="refFormObserver">
      ...
      <b-form @submit.prevent="handleSubmit(reply(file_name3))">
 ...
 <b-button size="sm" :disabled="!contactPerson_hasNextPage" @click="offset += limit">Next</b-button>
  ...
  <b-form-input id="email-subject" v-model="composeData.subject" />
        ...
        <div
          @click="if (showCcField) { (showCcField = !showCcField) } if (showBccField) { (showBccField = !showBccField) }"
          class=" pb-0 mt-0 mb-0 pt-0" style="margin-left:-1rem;">
          <!-- <div    > -->



          <quill-editor id="quill-content2" v-model="composeData.message" :options="editorOption" />
          <!-- <feather-icon class="ml-2 cursor-pointer" id="forward-content-button" @click="hide_parent_message()" icon="MoreHorizontalIcon" size="20" style="display:none"></feather-icon> -->

          <feather-icon class="ml-2 cursor-pointer" id="forward-content" @click="show_parent_message()"
            icon="MoreHorizontalIcon" size="20"></feather-icon>

          <!-- </div> -->
          <div id="forward-parent-message" v-if="emailViewData.childs" class="text-left pl-1" style="display: none;">
            <div class="text-right">

              <feather-icon class=" cursor-pointer" @click="hide_parent_message()" icon="TrashIcon" size="18" />
            </div>
            <iframe src=""
              :srcdoc="emailViewData.childs ? emailViewData.childs[emailViewData.childs.length - 1].message : ''"
              style="width:100%; height:auto;" frameborder="0"></iframe>

          </div>
          <div id="forward-parent-message" v-if="!emailViewData.childs" class="text-left pl-1" style="display: none;">
            <iframe src="" :srcdoc="emailViewData.parent ? emailViewData.parent.message : ''"
              style="width:100%; height:auto;" frameborder="0"></iframe>
          </div>
        </div>
        ...
        <b class="ml-1 d-flex justify-content-between"
          v-if="($store.state.tasks.s3_url.length > 0 && !$store.state.emails.isLoadingUrl)"
          ...
          <span v-b-tooltip.hover.top="'Remove Attachment'">
            ...
            <feather-icon class="mr-1 cursor-pointer" @click="removeFileS3(name.url)"
              v-if="($store.state.tasks.s3_url.length > 0 && !$store.state.emails.isLoadingUrl)" icon="XIcon" />
            <!-- </b-button> -->
          </span>
        </b>
        ...
        <div id="reply_actions" style="display: flex;">
          ...
          <div id="quill-toolbar3" class="d-flex border-bottom-0">
            ...
            <!-- &emsp13;<a href="#" class="text-decoration-none " style="color: #202126;" @click="uploadFileS3(file_name3)">Upload</a> -->
            <div class="text-center">
...
export default {
  ...
  methods: {
    assignfrom() {
      ...
      if(this.emailViewData.childs){
        childs = this.emailViewData.childs
      }
      var data = {}
      if(this.emailViewData.parent){
        data = this.emailViewData.parent
      }
      ...
      const daynames = ["Mon", "Tues", "Wed", "Thu", "Fri", "Sat",
        "Sun"
      ];
      if (childs.length > 0) {
        var child_subject = childs[childs.length - 1].subject
        var child_toEmail = childs[childs.length - 1].to_email
        var date = new Date(childs[childs.length - 1].date)
        var forwarded_mesage = "<p>----------Forwarded message---------</p><p>From: " + this.composeData.from.email + "</p><p>Date: " + daynames[date.getDay()] + ' , ' + monthNames[date.getMonth()] + ' ' + date.getDate() + ', ' + date.getFullYear() + ' at ' + date.getHours() + ':' + date.getMinutes() + "</p><p>Subject: " + child_subject + "</p><p>To: " + child_toEmail + "</p><br>"
        
        this.composeData.message = forwarded_mesage + childs[childs.length - 1].message
      } else {
        if (data.message) {
          var parent_message = data.message
          var parent_subject = data.subject
          var parent_toEmail = data.to_email
          
          var date = new Date(data.date)

          var forwarded_mesage = "<p>----------Forwarded message---------</p><p>From: " + this.composeData.from.email + "</p><p>Date: " + daynames[date.getDay()] + ' , ' + monthNames[date.getMonth()] + ' ' + date.getDate() + ', ' + date.getFullYear() + ' at ' + date.getHours() + ':' + date.getMinutes() + "</p><p>Subject: " + parent_subject + "</p><p>To: " + parent_toEmail + "</p><br>"
          this.composeData.message = forwarded_mesage + parent_message
    ...
    reply(file) {
      ...
      const data = {
        ...
        subject: latest_element.subject.includes('Fwd:') ? latest_element.subject : 'Fwd:' + latest_element.subject,
    ...
    hidereply() {
      ...
      this.discardEmails()
      if (document.getElementById('forward-div').style.display == 'block') {

        document.getElementById('forward-div').style.display = 'none';
      }
    ...
    show_parent_message() {
      ...
      // if(document.getElementById('forward-content').style.display == 'block'){
      // document.getElementById('forward-content-button').style.display = 'none';

      document.getElementById('forward-content').style.display = 'none';
      // }
      if (document.getElementById('forward-parent-message').style.display == 'none') {

        document.getElementById('forward-parent-message').style.display = 'block';
      }
    ...
    hide_parent_message() {
      ...
      // if(document.getElementById('forward-content').style.display == 'none'){

      document.getElementById('forward-content').style.display = 'block';
      // }
      if (document.getElementById('forward-parent-message').style.display == 'block') {

        document.getElementById('forward-parent-message').style.display = 'none';
      }
    ...
    discardEmails() {
      ...
      if (this.file_name3.length > 0) {

        while (this.file_name3.length > 0) {
          this.file_name3.pop();
        }
      }
    ...
    onFileChange(e) {
      console.log(this.file_name3, 'ccccccccccccccccccccccccccccccccccccccccc')
      ...
      image.forEach((x, index) => {
        ...
        reader.onload = (e) => {
          ...
          this.file_name3.push({
            file: e.target.result,
            name: x.name
          })
          this.uploadFileS3(this.file_name3)
        }
        // const element = array[index];
  ...
  //       if(this.emailViewData.parent.ccaddress != null ?? this.emailViewData.parent.ccaddress.includes(',') ){
  ...
  //          if(this.original_cc[i]!= null ?? this.original_cc[i].includes('<')){
  ...
  //          // if(this)
  //          if ((this.original_cc[i] != null ?? this.original_cc[i].includes('<')) && !(this.check_cc.includes(this.original_cc[i]))) {
  //           //  alert('jj')
  ...
  //     }
  //     this.giveCC = latest_element.trim().split(',');
  //         }
  //     if (this.emailViewData.childs) {
