<template>
  <div>
   ...
   <h5 class="mb-0">
        ...
        <div class="d-flex justify-content-between align-items-center">
          ...
          >
            <span
              ...
              "
              title="New Google Document"
              class="waves-effect action"
              ><img :src="wordIcon" width="25px" data-type="2"
            /></span>
            <span
              v-b-modal.attachment-excel-modal
              @click="
                () => {
              ...
              title="New Google Spreadsheet"
              class="waves-effect action"
              ><img :src="excelIcon" width="25px" data-type="1"
            /></span>

            <span title="Browse" class="waves-effect action"
              ><img :src="browseIcon" width="25px"
            /></span>

            <span
              v-b-modal.comment-compose-modal
              @click="setTaskComments()"
              title="Email"
              class="waves-effect action"
              style="width: 35px; height: 35px;"
              ><img :src="gmailIcon" width="25px"
            /></span>
          </div>
        </div>
      ...
      />
        ...
        <section class="chat-app-window">
          ...
          <vue-perfect-scrollbar
            ...
            style="padding-top: 2rem; background-color: #fff !important;"
          >
            <chat-log
              :reply="message_reply"
          ...
          <!-- Message Input -->
          <chat-form-vue
            v-if="client.status_master.type != 'closed'"
      ...
      <div
        v-if="$store.state.comments.isLoading"
      ...
      >
        ...
        >
          <span aria-hidden="true" class="spinner-border text-primary">
            <!----></span
          >
          <br /><span>Processing..</span>
        </div>
      </div>
    </div>
    <b-modal
      no-close-on-backdrop
      id="comment-compose-modal"
      ref="comment-compose-modal"
      size="lg"
      hide-footer
    >
        ...
        <b-col md="12">
          <div data-v-22d84313="" class="list-group">
            ...
            <div data-v-22d84313="" class="list-group-item">
              <div
                data-v-22d84313=""
                class="demo-checkbox d-flex align-items-baseline"
              >
                <input
                  data-v-22d84313=""
                  type="checkbox"
                  id="md_checkbox_task_description"
                  v-model="commentCompose.task_desc.check"
                  class="filled-in chk-col-theme"
                  :value="commentCompose.task_desc.text"
                />
                <label
                  data-v-22d84313=""
                  for="md_checkbox_task_description"
                  class="ml-1"
                  style="overflow: hidden; color: #777 !important;"
                  >{{
                    commentCompose.task_desc.text != null
                      ? commentCompose.task_desc.text.replace(
                          /(<([^>]+)>)/gi,
                          ""
                        )
                      : ""
      ...
      <b-row :class="{ 'mt-4': commentCompose.task_desc.text }">
        <b-col md="12">
          <div data-v-22d84313="" class="list-group">
            ...
            <div
              data-v-22d84313=""
              class="text-center list-group-item chat-history"
            >
              <div
                style="max-height: 260px; overflow-y: auto; overflow-x: hidden;"
              >
                <b-row
                  v-for="(c, index) in commentCompose.task_comments"
                  :id="String(index + 1)"
                  :key="c.comment"
                >
                  <b-col md="1">
                  ...
                  <b-col md="11">
                    <div
                      :class="{
                        'float-right': c.user_id == user_id,
                        'float-left': c.user_id != user_id,
                      }"
                    >
                      <div
                        :class="{
                          'text-left': c.user_id != user_id,
                          'text-right': c.user_id == user_id,
                        }"
                        class="message-data"
                      >
                        <span>
                      ...
                      <div
                        :class="{
                          'bg-me': c.user_id == user_id,
                          'bg-other': c.user_id != user_id,
                        }"
                        style="
                          font-size: 13px;
                          padding: 5px 15px;
                        ...
                        "
                        class="message my-message text-left"
                      >
                        <div
                          style="max-width: 400px;"
                          v-html="showComment(c)"
                        ></div>
                        <!-- {{ }} -->
                        <span
                          style="color: red;"
                          v-if="textLength == 'false' && c.comment.length > 100"
                        ...
                        <span
                          ...
                          style="color: red;"
                          v-if="textLength == 'true' && c.comment.length > 100"
      ...
      <b-row class="mt-4">
        <b-col md="6">
          <div data-v-22d84313="" class="list-group">
            ...
            <div
              v-if="taskAttachments.filter((e) => e.type === 'task').length > 0"
              data-v-22d84313=""
              class="list-group-item text-center col-grey"
            >
              <b-row
                v-for="p in taskAttachments.filter((e) => e.type === 'task')"
                :key="String(Number(p.id + 1))"
              >
                <b-col md="12">
                  <div class="float-left">
                    <input
                      type="checkbox"
                      @click="setEmailAttach()"
                      v-model="p.checked"
                      name=""
                      id=""
                    />
            ...
            <div
              v-if="
                taskAttachments.filter((e) => e.type === 'task').length === 0
              "
              data-v-22d84313=""
              class="list-group-item text-center col-grey"
            >
        ...
        <b-col md="6">
          <div data-v-22d84313="" class="list-group">
            ...
            <div
              v-if="
                taskAttachments.filter((e) => e.type === 'company').length === 0
              "
              data-v-22d84313=""
              class="list-group-item text-center col-grey"
            >
            ...
            >
              <b-row
                v-for="p in taskAttachments.filter((e) => e.type === 'company')"
                :key="String(Number(p.id + 1))"
              >
                <b-col md="12">
                  <div class="float-left">
      ...
      <b-row class="mt-4">
        <b-col md="6" class="d-none">
          <div data-v-22d84313="" class="list-group">
            ...
            <div
              v-if="$store.state.proposals.allProposals.length === 0"
              data-v-22d84313=""
              class="list-group-item bg-shaded-grey"
            >
            ...
            <div
              ...
              v-if="$store.state.proposals.allProposals.length > 0"
              class="list-group-item text-center col-grey"
              style="max-height: 160px; overflow: hidden auto;"
            >
              <!-- No proposal(s) available -->
              <b-row
                v-for="p in $store.state.proposals.allProposals"
                :key="String(p.id)"
              >
                <b-col md="12">
                  <div class="float-left">
        ...
        <b-col md="6" class="d-none">
          <div data-v-22d84313="" class="list-group">
            ...
            <div
              data-v-22d84313=""
              v-if="$store.state.invoice.all.length > 0"
              class="list-group-item text-center col-grey"
              style="max-height: 160px; overflow: hidden auto;"
            >
              <!-- No proposal(s) available -->
              <b-row v-for="i in $store.state.invoice.all" :key="String(i.id)">
                <b-col md="12">
                  <div class="float-left">
                    <input type="checkbox" name="" id="" />
                    <label
                      style="display: inline-grid; margin-left: 0.5rem;"
                      for="
                      "
                    >
                      <!-- {{ getPL(p.created_at,p.id) }} -->
        ...
        <b-col md="6" class="d-none">
          <div data-v-22d84313="" class="list-group">
            ...
            >
              ...
              <b-row
                v-for="payment in $store.state.recordPayment.all"
      ...
      <div class="text-right">
        <b-button
          size="sm"
          class="mr-1"
          @click="$refs['comment-compose-modal'].hide()"
          variant="primary"
          >Cancel</b-button
        >
        <b-button
    ...
    >
      <b-form @submit.prevent="sendEmail()">
            ...
            <div class="border-l">
                ...
                >
                  ...
                  <div>
                    <input
                      ...
                      style="display: none;"
                    />
                    <b-button size="sm" variant="primary" @click="showFile()"
              ...
              >
                <div v-if="$store.state.tasks.s3_url.length > 0">
                  <div
                    v-for="(file, index) in $store.state.tasks.s3_url"
                    :key="index"
                ...
                <div
                  v-if="
                    taskAttachments.filter(
                      (e) => e.checked === true && e.type === 'task'
                    ).length > 0
                  "
                >
                  <span
                    v-for="(file, index) in taskAttachments.filter(
                      (e) => e.checked === true && e.type === 'task'
                    )"
                    :key="file.id"
                    :id="String(Number(file.id + 1))"
                  >
                    <span v-if="file.checked === true">
                      {{ file.original_name }}
                      <feather-icon
            ...
            <div class="border-l">
              ...
              >
                <div
                  v-if="
                    taskAttachments.filter(
                      (e) => e.checked === true && e.type === 'company'
                    ).length > 0
                  "
                >
                  <span
                  ...
                  >
                    <span
                      v-if="file.checked === true && file.type === 'company'"
                    >
                      {{ file.original_name }}
                      <feather-icon
                        icon="XCircleIcon"
        ...
        <div class="text-right">
          <b-button
            size="sm"
            @click="$refs['reply-comment-modal'].hide()"
            variant="primary"
            class="mr-1"
          >
            Cancel
...
export default {
      ...
      total_audits.forEach((element) => {
        ...
        if (element.new_values.subject && element.old_values.subject) {
          desc =
            desc +
            "<div><b>Subject: </b>" +
            element.old_values.subject +
            " <b>Changed to </b>" +
            element.new_values.subject +
            "</div> <br>"
        }
        if (element.new_values.branch_id && element.old_values.branch_id) {
          desc =
            desc +
            "<div><b>Branch: </b>" +
            this.$store.state.branches.all.filter(
              (e) => e.id === element.old_values.branch_id
            )[0].name +
            "<b> Changed to </b>" +
            this.$store.state.branches.all.filter(
              (e) => e.id === element.new_values.branch_id
            )[0].name +
            "</div> <br>"
        }
        if (element.event != "created") {
          if (
            element.old_values.category_id !== null &&
            element.new_values.category_id === null
          ) {
            desc = desc + "<div><b>Category Removed</b></div>"
          }
          if (
            (element.new_values.category_id &&
              element.old_values.category_id) ||
            (element.new_values.category_id &&
              element.old_values.category_id == null)
          ) {
            if (element.old_values.category_id == null) {
              desc =
                desc +
                "<div><b>Category: </b> <b>Changed to </b>" +
                this.$store.state.categories.all.filter(
                  (e) => e.id === element.new_values.category_id
                )[0].name +
                "</div> <br>"
            } else {
              desc =
                ...
                "<div><b>Category: </b>" +
                this.$store.state.categories.all.filter(
                  (e) => e.id === element.old_values.category_id
                )[0].name +
                " <b>Changed to </b>" +
                this.$store.state.categories.all.filter(
                  (e) => e.id === element.new_values.category_id
        ...
        if (element.event != "created") {
          if (
            (element.new_values.company_id && element.old_values.company_id) ||
            (element.new_values.company_id &&
              element.old_values.company_id == null)
          ) {
            if (element.old_values.company_id == null) {
              desc =
                desc +
                "<div><b>Client: </b> <b>Changed to </b>" +
                this.$store.state.clients.all.filter(
                  (e) => e.id === element.new_values.company_id
                )[0].name +
                " </div><br>"
            } else {
              desc =
                ...
                "<div><b>Client: </b>" +
                this.$store.state.clients.all.filter(
                  (e) => e.id === element.old_values.company_id
                )[0].name +
                " <b>Changed to </b>" +
                this.$store.state.clients.all.filter(
                  (e) => e.id === element.new_values.company_id
        ...
        if (element.event != "created") {
          if (
            (element.new_values.contact_person_id &&
              element.old_values.contact_person_id) ||
            (element.new_values.contact_person_id &&
              element.old_values.contact_person_id == null)
          ) {
            if (element.old_values.contact_person_id == null) {
              desc =
                desc +
                "<div><b>Contact Person: </b> <b>Changed to </b>" +
                this.$store.state.contactPeople.all.filter(
                  (e) => e.data.id === element.new_values.contact_person_id
                )[0].data.name +
                "</div>"
            } else {
              desc =
                desc +
                "<div><b>Contact Person: </b>" +
                this.$store.state.contactPeople.all.filter(
                  (e) => e.data.id === element.old_values.contact_person_id
                )[0].data.name +
                " <b>Changed to </b>" +
                this.$store.state.contactPeople.all.filter(
                  (e) => e.data.id === element.new_values.contact_person_id
        ...
        if (element.old_values.description && element.new_values.description) {
          desc =
            desc +
            "<div><b>Description: </b></div><div>" +
            element.old_values.description +
            "</div><div><b> Changed to </b></div><div>" +
            element.new_values.description +
            "</div> <br>"
        }
        if (
          element.new_values.due_date &&
          !element.new_values.category_id &&
          element.old_values.due_date &&
          !element.old_values.category_id
        ) {
        ...
        if (element.new_values.due_date_reason) {
          //(element.new_values.due_date && element.new_values.due_date_reason) && (element.old_values.due_date && element.old_values.due_date_reason)) {
          // if(this.$router.currentRoute.name == 'tasks'){

          desc =
            desc +
            "<div><b>Due Date Reason: </b>" +
            element.new_values.due_date_reason +
            "</div><br>" //' <b>Changed to </b>' + element.new_values.due_date + '</div> <br>'
          // }else{
          //   desc = desc + '<br> <b>Due Date: </b>'+ element.old_values.due_date+ ' <b>Changed to </b>'+ this.$store.state.leads.all.filter(e => e.type === 'lead').filter(e => e.due_date == element.new_values.due_date)[0].due_date
        ...
        // }
        if (
          element.new_values.invoice_date &&
          element.old_values.invoice_date
        ) {
          // if(this.$router.currentRoute.name == 'tasks'){
        ...
        if (
          element.new_values.invoice_number &&
          element.old_values.invoice_number
        ) {
          // if(this.$router.currentRoute.name == 'tasks'){
        ...
        if (
          element.new_values.proposal_date &&
          element.old_values.proposal_date
        ) {
          // if(this.$router.currentRoute.name == 'tasks'){

          desc =
            desc +
            "<div><b>Proposal Date: </b>" +
            element.old_values.proposal_date +
            " <b>Changed to </b>" +
            element.new_values.proposal_date +
            "</div> <br>"
        ...
        if (element.new_values.to_do && element.old_values.to_do) {
          desc =
            desc +
            "<div>" +
            element.old_values.to_do +
            " <b>Changed to</b> " +
            element.new_values.to_do +
            "</div> <br>"
        }
        if (
          element.old_values.status &&
          element.new_values.status &&
          element.auditable_type.includes("Task")
        ) {
          desc =
            desc +
            "<div><b> Status: </b>" +
        ...
        if (element.event != "created") {
          if (
            (element.new_values.status_master_id &&
              element.old_values.status_master_id) ||
            (element.new_values.status_master_id &&
              element.old_values.status_master_id == null)
          ) {
            ...
            if (
              element.old_values.status_master_id == null &&
              element.new_values.status_master_id != null
            ) {
              desc =
                desc +
                "<b>Status: </b> <b>Changed to </b>" +
                this.$store.state.status.all.filter(
                  (e) => e.id === element.new_values.status_master_id
                )[0].type +
                " <br>"
            }
            if (
              ...
              desc =
                ...
                "<div><b>Status: </b>" +
                this.$store.state.status.all.filter(
                  (e) => e.id === element.old_values.status_master_id
                )[0].type +
                " <b>Changed to </b>" +
                this.$store.state.status.all.filter(
                  (e) => e.id === element.new_values.status_master_id
        ...
        if (desc || message === "Created" || message === "Added") {
          all.push({
            comment:
              ...
              message +
              " " +
              (!element.auditable_type.includes("ToDo")
                ? type.split("Models\\")[1]
                : "sub-task") +
              "</p>" +
              ((element.event != "created" && element.event != "deleted") ||
              element.auditable_type.includes("ToDo")
                ? '<hr style="border:1px solid #71ccf7;"/>'
                : "") +
              // || (element.auditable_type.includes('TaskFollowUp')) ? '<hr style="border:1px solid #71ccf7;"/>' : '')
              "<i>" +
              (element.event == "created" &&
              element.auditable_type.includes("Task")
                ? ""
                : desc) +
              "</i>",
  ...
  methods: {
    ...
    replyModal() {
      ...
      if (this.extraComments.length > 0) {
        thread =
          thread +
          "<br><br><br>" +
          this.extraComments[this.extraComments.length - 1].message
      } else {
        thread = thread + ""
      }
      if (this.$store.state.comments.extraComment.length > 0) {
        thread =
          thread +
          "<br><br><br>" +
          this.$store.state.comments.extraComment[
            this.$store.state.comments.extraComment.length - 1
          ].message
      } else {
        thread = thread + ""
      }
      ...
      if (this.$router.currentRoute.name == "leads") {
        header =
          ...
          this.$store.state.leads.all
            ...
            .filter((e) => e.id === this.task_id)[0].contact_person.name +
          ", Greeting from " +
      ...
      let comment_arr = this.commentCompose.task_comments.filter(
        (e) => e.checked === true
      )
      let comment_content =
        '<h2 style="color: black;background: white;">Comments</h2><br>'
      if (comment_arr.length > 0) {
        comment_arr.forEach((element) => {
          comment_content =
            comment_content +
            '<p style="color: black;background: white;"><b>' +
            element.user.name +
            ": </b><br>" +
            element.comment +
            "</p> "
      ...
      if (signAll.length > 0) {
        this.composeData.message =
          header + comment_content + message + signAll[0].signature + thread
      }
      ...
      this.emails_contact.forEach((element) => {
        if (this.client.mailbox != null) {
          if (
            element == this.client.mailbox.from_email &&
            this.client.mailbox.folder == "INBOX"
          ) {
            this.composeData.cc.push(element)
          }
        }
        this.client.contact_person.emails.forEach((e) => {
          if (element == e.email) {
    ...
    showSearch() {
      if (
        (document.querySelector("input[id=searchBar]").style.display = "none")
      ) {
    ...
    sendMessage(message) {
      ...
      this.$store
        .dispatch("comments/store", { payload: comment, id: this.task_id })
        .then(() => {
          this.fetchAll()
        })
