        ...
        <b-row>
          ...
          >
            <b-form-group   label="Contact Person *">
              <v-select :disabled="editItem.status_master.type !== 'open'" v-model="formData.contact_person_id" :options="contactPeople" label="name"
...
<!-- <b-form-group v-if="$router.currentRoute.name == 'tasks'"  label="Contact Person">
    ...
    item.name.substring(0, 28)
          ...
          <b-col :md="setMd(branches.length)">
              <validation-provider #default="validationContext" name="company_id" vid="client" rules="required"
                ...
                <b-form-group label="Client *">
                  <v-select :disabled="editItem.status_master.type !== 'open'" id="client" :options="clients" label="name" v-model="formData.company_id"
              ...
              <b-form-group v-if="$router.currentRoute.name == 'leads'" label="Client">
                <v-select :disabled="editItem.status_master.type !== 'open'" id="client" :options="clients" label="name" v-model="formData.company_id"
                  placeholder="Search Client">
                  <template #list-header class="add_clients">
                    <div class=" ml-2 mb-1 mt-1" v-b-modal.modal-clients>
          ...
          <b-col :md="setMd(branches.length)">
            <b-form-group label="Category">
              <v-select :disabled="editItem.status_master.type !== 'open'" placeholder="Search Category" v-model="formData.category_id" label="name" :options="categories"
                @input="categoryUser" >
                <template #option="item">
                    <span v-b-tooltip.hover.top="item.name">
          ...
          <b-col :md="setMd(branches.length)" v-if="(branches.length > 1)">
            <validation-provider #default="validationContext" name="branch_id" vid="branch" rules="required">
              ...
              <b-form-group label="Branch *">
                <v-select :disabled="editItem.status_master.type !== 'open'" id="branch" placeholder="Search Branch"
          ...
          <b-row>
            <b-col md="12 ">
              <b-form-group label="Subject *" label-for="contact">
                <b-form-input :disabled="editItem.status_master.type !== 'open'" placeholder="Enter Subject" id="subject" v-model="formData.subject">
        ...
        <b-row>
          <b-col md="12 ">
            <b-form-group label="Description" label-for="contact">
              <quill-editor :disabled="editItem.status_master.type !== 'open'" v-model="formData.description" :options="editorOption">
        ...
        <b-row>
          <b-col md="3">
            <b-form-group label="Assigned To">
              <v-select :disabled="editItem.status_master.type !== 'open'" multiple placeholder="Search Employee" label="name" v-model="formData.users"
                :options="allUsers">
    ...
    item.name.substring(0, 28)
          ...
          <b-col md="3">
            <validation-provider #default="validationContext" name="due_date" vid="due_date" rules="required">
              ...
              <b-form-group label="Due Date *">
                <VueDatePicker :disabled="editItem.status_master.type !== 'open'" format="DD-MM-YYYY" @input="openReason()" style="border:1px solid #d8d6de;border-radius: 0.357rem;" v-model="formData.due_date" :minDate="currentDate" />
          ...
          <b-col md="3">
            <validation-provider #default="validationContext" name="priority" vid="priority" rules="required">
              ...
              <b-form-group label="Priority *">
                <v-select :disabled="editItem.status_master.type !== 'open'" autofocus v-model="formData.priority"
          ...
          <b-col md="3">
        <validation-provider #default="validationContext" name="status" vid="subject" rules="required">
...
let year = date.getFullYear();
  ...
  computed: {
    ...
    statusMaster() {
      ...
      let index = items.findIndex(e => e.type === 'invoiced')
      items.splice(index,1)
      var status = []
      for (let index = 0; index < items.length; index++) {
        // if(items[index].type !== 'invoiced'){
  ...
  methods: {
    openReason(){
      this.$store.state.leads.due_date = this.formData.due_date
      this.$bvModal.show('modal-due-date')
    },
    resetFormData() {
    ...
    onSubmit() {
      ...
      if(this.$router.currentRoute.name === 'tasks'){
        ...
        if(!this.formData.status || !this.formData.branch_id || !this.formData.company_id || !this.formData.due_date || !this.formData.priority)
        {
          return;
        }
      }else {
        if(!this.formData.status  || !this.formData.branch_id || !this.formData.contact_person_id || !this.formData.due_date || !this.formData.priority)
        {
          return;
        }
      }
    ...
    categoryUser(data) {
      ...
      if (date != null) {
        ...
        this.formData.due_date = d.toISOString().slice(0, 10);
      }
