<template>
  ...
  <div>
     {{ defaultEmail }}
    <b-modal no-close-on-backdrop id="modal-leads" ref="modal-leads" size="lg" hide-footer
      @hidden="resetAttachment" class="b-overlay position-relative"
    >
      <!-- <b-card-body> -->
      <template #modal-header>
        <h5 class="modal-title">
          ...
          <span v-if="$router.currentRoute.name == 'dashboard'">
            add new lead
          </span>
          <span v-if="$router.currentRoute.name == 'GettingStarted'">
            add new lead
          </span> -->
      ...
      <validation-observer #default="{ handleSubmit }" ref="refFormObserver">
        ...
        <b-form @submit.prevent="handleSubmit(onSubmit)">
          ...
          <b-row>
            <b-col v-if="!isChecked" :md="setMd(branches.length)">
              <validation-provider #default="validationContext" name="contact_person_id" vid="contact" rules="required"
                >

                <b-form-group label="Contact Person *"   class="my-custom-select">
                  <v-select  id="contact" v-model="formData.contact_person_id" :options="paginated_contact" label="name"
            ...
            <b-col v-if="!isChecked" :md="setMd(branches.length)">
                ...
                <b-form-group label="Client "   class="my-custom-select">
                  <v-select id="client"  :options="paginated_clients" label="name" v-model="formData.company_id"
            ...
            <b-col  v-if="!isChecked" :md="setMd(branches.length)">
              <b-form-group label="Category"   class="my-custom-select">
                <v-select placeholder="Search Category"  v-model="formData.category_id" label="name" 
                  :options="paginated_categories" @input="categoryUser">
                  <template #option="item">
                    <span v-b-tooltip.hover.top="item.name">
            ...
            <b-col :md="setMd(branches.length)" v-if="(branches.length > 1) || isChecked">
              <validation-provider #default="validationContext" name="branch_id" vid="branch" rules="required">

                <b-form-group label="Branch *"   class="my-custom-select">
                  <v-select   id="branch" placeholder="Search Branch"
                    v-model="formData.branch_id" label="name"  :options="paginated_branches">
                    <template #option="item">
                      <span v-b-tooltip.hover.top="item.name">
          ...
          <validation-provider #default="validationContext" name="subject" vid="subject" rules="required">
            <b-row>
              <b-col md="12 ">
                <b-form-group label="Subject *" label-for="contact" class="my-custom-select">
            ...
            <b-col :md="setSpecialMd()">
              <validation-provider #default="validationContext" name="due_date" vid="due_date" rules="required">
                ...
                <b-form-group label="Due Date *">
                  <VueDatePicker format="DD-MM-YYYY" style="border:1px solid #d8d6de;border-radius: 0.357rem;" v-model="formData.due_date" :minDate="currentDate" />
              ...
              <validation-provider #default="validationContext" name="priority" vid="priority" rules="required">
                ...
                <b-form-group label="Priority *" class="my-custom-select">
                  <v-select   autofocus v-model="formData.priority"
                    label="id" :options="importance.slice().reverse()" input-id="priority"
              ...
              <b-row class="mb-1">
                <b-col md="12" class="d-flex">
                  <div style="width: 60%;">
                    ...
                    <validation-provider #default="validationContext" name="default_todo" vid="default_todo"
                      rules="">
                      <b-form-group >
                    ...
                    <div class="border-l" style="">
                      <div class="d-flex flex-column">
                        <div class="d-flex justify-content-between align-items-center p-1 border-bottom-light bg-light">
                          <div>Attachment </div>
                          <div>
                            <input type="file" ref="file_task2" style="display: none" @change="onFileChange"/>
                            <b-dropdown id="file_drop" class="" style="width:50%;" variant="primary" size="sm">
                              ...
                              <b-dropdown-item @click="$refs.file_task2.click()"><img class="mr-1"
                                  :src="browseIcon"
                      ...
                      <div class="d-flex justify-content-between align-items-center p-1" >
                        <div  v-if="file_name3 " class="d-flex justify-content-between">
                          <div v-for="(file, index) in fileLinks" :key="index"
                                class="d-flex justify-content-between">

                                  <span v-if="file_name3.name === file.fileName">
                                  {{ file.fileName }}
               ...
               > -->
                  <div style="width:60%">
                    ...
                    <validation-provider #default="validationContext" name="additional_todo" vid="additional_todo"
                      rules="">
                      <Mentionable style="width: 100%;" :keys="['@']" :items="users_to_assign" offset="6" insert-space
                        @open="onOpen" @apply="onApply" class="my-custom-select">
                      ...
                      <div class="d-flex flex-column">
                        <div class="d-flex justify-content-between align-items-center p-1 border-bottom-light bg-light">
                          ...
                          <div>
                            ...
                            <b-dropdown id="file_drop" class="" size="sm" style="width:50%;" variant="primary">
                              <template #button-content>
                                <span class="text-capitalize">
                              ...
                              <b-dropdown-item @click="$refs.file_task3.click()"><img class="mr-1"
                                  :src="browseIcon"
                                  width="25px">Browse</b-dropdown-item>
                              <b-dropdown-item><img class="mr-1"
                                  :src="excelIcon"
            ...
            <div class="border-l" style="width: 40%;">
              <div class="d-flex flex-column">
                <div class="d-flex justify-content-between align-items-center p-1 border-bottom-light bg-light">
                  <div>Attachment(s) </div>
                  <div>
                    <input type="file" multiple ref="file_task" @change="onFileChange2"  style="display: none" />
                    <b-dropdown id="file_drop" class="" size="sm" style="width:50%;" variant="primary">
                      <template #button-content>
                        <span class="text-capitalize">
              ...
              <div class="p-1">
                              ...
                              <div v-if="file_name2.length > 0">
                                <div v-for="(file, index) in fileLinks" :key="index"
                                >
                               <!-- <span v-if="file.type === 'attachmentsType'"> -->
                                <div v-for="i in file_name2">

                                  <span v-if="file.fileName === i.name">
      ...
      <!-- </b-card-body> -->
      <div v-if="$store.state.leads.isSidebarLoading" class="b-overlay position-absolute"
...
this.formData.users =  []//this.$store.state.users.all.filter(e => e.email === JSON.parse(localStorage.getItem('userData')).email)
    ...
    paginated_branches(){
     ...
     return this.branches.slice(0,0+8)
    },
    clients() {
      
      return this.$store.state.clients.all.filter((country) =>
        country.name.toLocaleLowerCase().includes(this.search.toLocaleLowerCase()))
    },
    paginated_clients(){
    ...
    contactPeople() {
      ...
      return this.$store.state.contactPeople.allForLeads.filter((country) =>
        country.name.toLocaleLowerCase().includes(this.search.toLocaleLowerCase()))
    },
    paginated_contact(){
    ...
    allUsers() {
      ...
      //   this.$store.dispatch('allUsers/fetch');
      // }
      let data=this.$store.state.users.all.filter(e => e.status ==='active')
      // data.forEach(e =>{
    ...
    paginate_allUsers(){
     return this.allUsers.slice(0,0+8)
    }
  ...
  created() {
    ...
    if(this.$store.state.categories.all.length === 0){
  ...
  methods: {
    ...
    const mappedFiles = filePaths.map((base64File,image_name) => ({
      ...
      type:'subTaskType',
       ...
       if(this.file_name3){
        let index = this.fileLinks.findIndex((e)=>e.fileName === this.file_name3.name);
        this.fileLinks.splice(index,1);
        // let index2 = this.fileArrayMultiple.findIndex((e)=>e.name === this.file_name3.name);
        // this.file_name2.splice(index2,1);
      }
    this.file_name3 = mappedFiles[0];
    console.log("lllllllllllllllllllll", this.file_name3, "mmmmmmmmmmmm",mappedFiles);
     
      // if (this.file_name2.length === 0) {
      //   this.file_name2.push(this.file_name3);
      // } else {
      //   for (let i = 0; i < this.file_name2.length; i++) {
      //     if (this.file_name2[i].type === "subTaskType") {
      //       this.file_name2[i] = this.file_name3;
      //     } else {
      //       this.file_name2.push(this.file_name3);
      //     }
      //   }
    ...
    tempFile.forEach((obj2)=>{
      ...
      if(!exists){
        ...
        readyToUpload.push(obj2);
      }
    })
    
    console.log(mappedFiles);
    this.uploadFileS3(readyToUpload);
//       console.log("e:", image);
e.target.value = ''
...
// });
    ...
    uploadFileS3(file) {
      ...
      this.$store.dispatch('attachments/add', { data: this.data2 }).then(() => {
    ...
    resetFormData() {
      ...
      this.formData.description = '';
       this.formData.contact_person_id = this.$store.state.contactPeople.singleData.id?this.$store.state.contactPeople.singleData:this.paginated_contact[0];
      this.formData.company_id = null;
       this.formData.branch_id = this.branches[0];
    ...
    onSubmit() {
      ...
      if (this.branches.length === 1) {
        this.formData.branch_id = { b_id: this.$store.state.branches.all[0].id }
        proposal_branch_id = this.$store.state.branches.all[0].id
    ...
    //     client_name: this.formData.company_id != null ? this.formData.company_id.name : '',
    //     concerned_person: this.formData.contact_person_id != null ? this.formData.contact_person_id.name : null,
    //     address: proposal_branch_id != 0 ? this.$store.state.branches.all.find(e => e.id === proposal_branch_id).name : this.formData.branch_id.name,
    //     subject: this.formData.subject,
    ...
    //     client_id: this.formData.company_id != null ? this.formData.company_id.id : null
    //   }
    // }
      ...
      this.$store
        ...
        .then(() => {
          ...
          // emit("update:is-create-sidebar-active", false);
          this.$refs['modal-leads'].hide()
          this.$toast.success("Lead Created Successfuly!", {
        position: "bottom-left",
        timeout: 5000,
        closeOnClick: true,
        pauseOnFocusLoss: true,
