    ...
    <b-form @submit.prevent="onSubmit()">
      ...
      <div class="text-right">
        <b-button
          ...
          @click="
            () => {
              $refs['follow-up-modal'].hide(), resetFollowUp()
            }
          "
          variant="primary"
          class="mr-1"
        ...
        <b-button
          :disabled="$store.state.followUp.isLoading"
          type="submit"
          size="sm"
          variant="primary"
        >
          Create
          <b-spinner
            v-if="$store.state.followUp.isLoading"
            small
    ...
    >
      ...
      >
        <span aria-hidden="true" class="spinner-border text-primary">
          <!----></span
        >
        <br /><span>Adding New Follow Up..</span>
      </div>
    </div>
  </b-modal>
  <!-- </div> -->
  ...
  methods: {
    checkValid() {
      ...
      if (
        ...
        this.to24HourFormat(data.time) > this.to24HourFormat(this.currentTime)
      ) {
    ...
    onSubmit() {
      ...
      if (
        ...
        !data.subject ||
        (data.date === this.currentDate &&
          this.to24HourFormat(data.time) <
            this.to24HourFormat(this.currentTime))
      ) {
        return
      }
      if (data.follow_up && data.subject) {
        ...
        this.$store.dispatch("followUp/store", data).then(() => {
          this.$store
            .dispatch("followUp/get", this.$store.state.leads.expandedTaskId)
            .then(() => {
