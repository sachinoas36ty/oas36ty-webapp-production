    ...
    <b-row>
      <b-col md="3">
        <b-form-group label="From">
          ...
          <v-select v-model="composeData.from" :options="emails_contact" placeholder="Select From"></v-select>
        </b-form-group>
      ...
      <b-col md="3">
        <b-form-group label="Cc">
          ...
          <v-select multiple v-model="composeData.cc" :options="emails_contact" placeholder="Select Cc"></v-select>
        </b-form-group>
      ...
      <b-col md="3">
        <b-form-group label="Bcc">
          ...
          <v-select multiple v-model="composeData.bcc" :options="emails_contact" placeholder="Select Bcc"></v-select>
        </b-form-group>
...
export default {
  ...
  methods: {
    ...
    getFrom(){
      ...
      for (let i = 0; i < this.$store.state.contactPeople.allForLeads.length; i++) {

        // data[i] = this.$store.state.contactPeople.emails[i];
        if(this.$store.state.contactPeople.allForLeads[i].name == this.taskDetails.contact_person.name)
        {
          if(this.$store.state.contactPeople.allForLeads[i].emails.length > 0){
            email = this.$store.state.contactPeople.allForLeads[i].emails[0].email
          }
        }
      }
    }
