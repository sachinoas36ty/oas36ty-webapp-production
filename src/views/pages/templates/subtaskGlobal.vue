<template>
    <div>
        <b-modal no-close-on-backdrop id="template-modal-global" ref="template-modal-global" size="lg" :title="'Add new Template'"
hide-footer>
...
<validation-observer #default="{ handleSubmit }" ref="refFormObserver">
    ...
    createTemplate)">
                ...
                <validation-provider #default="validationContext" name="temp_name" vid="template"
                    ...
                    <b-form-group>
                        ...
                        <b-form-input v-model="temp_name" 
                            id="template" placeholder="Enter sub-task template name" />
            ...
            <b-col md="6">
                <b-form-group>
                    ...
                    <div class="border-l" style="">
                        <div class="d-flex flex-column">
                            <div
                                ...
                                <div>
                                    <input type="file" ref="file_task" @change="handleUpload"
                                        style="display: none" />
                                    <b-dropdown id="file_drop" class="" style="width:50%;"
                                        variant="primary">
                  ...
                  <div v-else-if="file_name.length">
                    ...
                    class="d-flex justify-content-between">{{ file.fileName }} <feather-icon icon="XIcon"
                ...
                <div class="text-right">
                     <b-button size="sm"   @click="addTemplate()" class="cursor-pointer"
        ...
        <b-row class="mt-1" v-for="(items, index) in templateFields" :key="items.template">
            <b-col md="6">
            ...
            <b-col md="6">
                <b-form-group>
                    ...
                    <div class="border-l" style="">
                        <div class="d-flex flex-column">
                            <div
                                ...
                                <div>Attachment(s)</div>
                                <div>
                                    <input type="file" id="file" @change="handleUpload2($event,index)"
                                        style="display: none" />
                                    <b-dropdown id="file_drop" class="" style="width:50%;"
                                        variant="primary">
                                        <template #button-content>
                                        ...
                                        <b-dropdown-item @click="openFile()"><img class="mr-1"
                                                :src="browseIcon"
                                                width="25px">Browse</b-dropdown-item>
                                        <b-dropdown-item><img class="mr-1"
                                                :src="excelIcon"
                  ...
                  <div v-else-if="items.file_name.length">
                    <div 
                    class="d-flex justify-content-between">{{ items.file_name[0].name }} <feather-icon icon="XIcon"
                      class="cursor-pointer" @click="removes3_from_addmore(items.file_name[0].name,index)" /></div>
                  </div>
                  <div v-else class="d-flex justify-content-center align-items-center">No file(s) selected
                  </div>
                        </div>
...
import { BAvatar, BDropdown, BDropdownItem, BMediaAside, BImg, BFormInvalidFeedback, BContainer, BSpinner, BRow, BTable, BCard, BCol, BButton, BModal, BFormInput, BFormGroup, BForm, } from 'bootstrap-vue'
import { ValidationProvider, ValidationObserver } from 'vee-validate'
import { Mentionable } from 'vue-mention'

export default {
    components:{
        BAvatar, BDropdown, BDropdownItem, BMediaAside, BImg, BFormInvalidFeedback, BContainer, BSpinner, BRow, BTable, BCard, BCol, BButton, BModal, BFormInput, BFormGroup, BForm,ValidationProvider, ValidationObserver ,Mentionable,
    },
    data(){
        return {
      ...
      this.$store.dispatch('attachments/add', { data: data }).then(() => {
        ...
        createTemplate() {
            ...
            arr.push({
                ...
                attachment_url:this.file_name.length > 0 ? this.fileLinks.find(e => e.fileName == this.file_name[0].name).url : null
            })
            this.templateFields.forEach(element => {
                arr.push({
                    steps_body:element.title,
                    attachment_url: element.file_name.length > 0  ? this.fileLinks.find(e => e.fileName == element.file_name[0].name).url : null
                })
            });
            ...
            this.$store.dispatch('subtaskTemplate/store', data).then(() => {
                this.$refs['template-modal-global'].hide()
...
<style>
#template-modal-global .modal-dialog {
    max-width: 880px;
}
