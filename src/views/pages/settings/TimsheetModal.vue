<template>
    <div>
        <!-- {{ setHours }} -->
        <b-modal class="b-overlay position-relative" no-close-on-esc no-close-on-backdrop id="timesheet-modal" ref="timesheet-modal" size="lg" title="add office timings"
            hide-footer>
            ...
            <div v-if="$store.state.tasks.isLoading" class="b-overlay position-absolute"
              ...
              <div class="position-absolute" style="top: 50%; left: 50%; transform: translateX(-50%) translateY(-50%);">
                ...
                <br><span style="margin-left:-2rem;">Fetching..</span>
              </div>
            </div>
        </b-modal>
    ...
    computed: {
        setLogs(){
            ...
            data.forEach(element => {
                ...
                //   const nowDate = new Date('2023-05-15')
                let date = JSON.parse(localStorage.getItem('userTimesheet')).latest_date
                if(element.created_at.split('T')[0] === date){
            ...
            if(taskId && subject && desc ){
                // if(element.created_at.split('T')[0] === this.latest_date){
