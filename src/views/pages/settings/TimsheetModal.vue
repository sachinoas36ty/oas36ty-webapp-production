<template>
  <div>
    ...
    <b-modal
      ...
      ref="timesheet-modal"
      size="lg"
      title="add office timings"
      hide-footer
    >
      <template #modal-header>
        <h5 class="modal-title">
          add time
          <small> {{ getDateMonth(latestDate2) }} - {{ currDate }} </small>
          ...
          <b-col>
            <div>
              <b-button
                ...
                "
                v-if="getDateMonth(latestDate2) === currDate"
                variant="primary"
              >
                Skip
              </b-button>
      ...
      <div class="table-responsive" style="max-height: 28rem;">
        <table class="table">
          ...
          <tbody id="body-append">
            <tr
              :id="'form-others' + Number(data.count + 1)"
              :key="String(Number(data.count) + 1)"
              v-if="multipleForms.length > 0"
              v-for="(data, index) in multipleForms.filter(
                (e) => e.type === 'others'
              )"
            >
              <td></td>
              ...
              <td>
                <b-form-input
                  required
                  @focus="data.old_values = data.time_devoted"
                  v-model="data.time_devoted"
                  @input="time_devotion"
                  placeholder="Enter Time Devoted"
                ></b-form-input>
                <div>
                  <b-button
                    size="sm"
                    @click="minusMin('form-others', data.count)"
                    variant="primary"
                    >-5m</b-button
                  >&emsp;
                  <b-button
                    size="sm"
                    @click="addMin('form-others', data.count, '5')"
                    variant="primary"
                    >+5m</b-button
                  >&emsp;
                  <b-button
                    size="sm"
                    @click="addMin('form-others', data.count, '15')"
                    variant="primary"
                    >+15m</b-button
                  >&emsp;
                  <b-button
                    size="sm"
                    @click="addMin('form-others', data.count, '30')"
                    variant="primary"
                    >+30m</b-button
                  >
                </div>
              ...
              <td>
                ...
                <feather-icon
                  ...
                  @click="deleteForms('form-others', index)"
                ></feather-icon>
                <feather-icon
                  icon="PlusCircleIcon"
                  size="20"
                  @click="AddForms('others')"
                  class="ml-1 cursor-pointer"
                ></feather-icon>
              </td>
            </tr>
            <tr
              :id="'select-task' + Number(data.count + 1)"
              :key="String(Number(data.count) + 1)"
              v-if="multipleForms.length > 0"
              v-for="(data, index) in multipleForms.filter(
                (e) => e.type === 'task'
            ...
            >
              ...
              <td>
                ...
                <div>
                  <b-button
                    size="sm"
                    @click="minusMin('select-task', data.count)"
                    variant="primary"
                    >-5m</b-button
                  >&emsp;
                  <b-button
                    size="sm"
                    @click="addMin('select-task', data.count, '5')"
                    variant="primary"
                    >+5m</b-button
                  >&emsp;
                  <b-button
                    size="sm"
                    @click="addMin('select-task', data.count, '15')"
                    variant="primary"
                    >+15m</b-button
                  >&emsp;
                  <b-button
                    size="sm"
                    @click="addMin('select-task', data.count, '30')"
                    variant="primary"
                    >+30m</b-button
                  >
                </div>
              ...
              <td>
                ...
                <feather-icon
                  ...
                  size="20"
                  class="cursor-pointer"
                  @click="deleteForms('select-task', index)"
                ></feather-icon>
            ...
            >
              ...
              <td>
                <feather-icon
                  icon="XCircleIcon"
                  @click="removeHoliday(index, 'half')"
                  size="20"
                />
              </td>
            </tr>
            ...
            >
              ...
              <td>
                <feather-icon
                  icon="XCircleIcon"
                  @click="removeHoliday(index, 'full')"
                  size="20"
                />
              </td>
            </tr>
            ...
            >
              ...
              <td>
                <feather-icon
                  @click="removeHoliday(index, 'holiday')"
                  icon="XCircleIcon"
                  size="20"
                />
              </td>
      ...
      <div class="d-flex justify-content-between">
        ...
        <div class="text-right">
          <b-button
            :disabled="$store.state.timesheet.isLoading"
    ...
    <b-modal
      ...
      ref="choose-tasks-modal"
      size="md"
      title="choose task/lead"
      hide-footer
    ...
    >
      ...
      <div class="text-right">
        ...
        <b-button
          @click="
            () => {
              AddForms('task'), $refs['choose-tasks-modal'].hide()
              tasks_data = null
            }
          "
          size="sm"
      ...
      <div
        v-if="$store.state.timesheet.isLoading"
      ...
      >
        ...
        >
          <span aria-hidden="true" class="spinner-border text-primary">
            <!----></span
          >
          <br /><span>Adding New Timesheet..</span>
        </div>
      </div>
    </b-modal>
  </div>
...
export default {
  ...
  data() {
    ...
    return {
      latestDate:
        JSON.parse(localStorage.getItem("userTimesheet")) != null
          ? JSON.parse(localStorage.getItem("userTimesheet")).latest_date
          : `${date < 10 ? "0" + date : date} ${monthArray[month]}`,
      latestDate2: null,
    ...
    setLogs() {
      ...
      data.forEach((element) => {
        ...
        if (element.auditable_type.includes("Task")) {
          ...
          desc = element.new_values.description
            ? element.new_values.description.replace(/(<([^>]+)>)/gi, "")
            : ""
        } else {
          if (element.new_values.task_id) {
            ...
            subject = this.$store.state.leads.all.filter((e) => e.id === taskId)
              .length
              ? this.$store.state.leads.all.filter((e) => e.id === taskId)[0]
                  .subject
              : null
            if (
              this.$store.state.leads.all.filter((e) => e.id === taskId).length
            ) {
              if (
                this.$store.state.leads.all.filter((e) => e.id === taskId)[0]
                  .description != null
              ) {
                desc = this.$store.state.leads.all
  ...
  methods: {
    ...
    updateTimesheet() {
      ...
      let half_valid = this.multipleForms.filter(
      ...
      if (half_valid.length > 0) {
        ...
        console.log("hehhe")
        if (
          this.latestDate2 !== current_date &&
          Number(this.totalHours.split(":")[0]) < 7
        ) {
        ...
        if (Number(this.totalHours.split(":")[0]) >= 24) {
          ...
          this.$swal(
            "",
            "Total time logged should be less than or equal to 23:59 hrs",
            "error"
          )
          return
        } else {
          ...
          console.log(data)
          this.$store
            .dispatch("timesheet/store", { timesheet: data })
            .then(() => {
              ...
              if (localData.latest_date !== b) {
                if (
                  time_check(localData.total_worked_hour) === "greater than"
                ) {
                  let abc = {
                ...
                if (
                  check_date_less_greater(
                    localData.latest_date,
                    localData.total_worked_hour
                  ) === "enable" &&
                  time_check(localData.total_worked_hour) === "less than"
                ) {
                  // alert('alert 2')
    ...
    time_devotion(ev) {
      ...
      const totalMinutes = timeArrays.reduce((total, timeArray) => {
        const timeMinutes = timeArray.reduce(
          (acc, [hours, minutes]) =>
