<template>
    <b-modal no-close-on-backdrop id="office-timings-modal" ref="office-timings-modal" size="lg" title="add office timings"
        hide-footer class="b-overlay position-relative">
            ...
            <b-form @submit.prevent="handleSubmit(createTimings)" class="">
                ...
                <b-row>
                    <b-col md="6">
                        ...
                        <validation-provider #default="validationContext" name="branches" vid="branches" rules="required">
                            ...
                            <b-form-group label="Branches *" class="my-custom-select">
                                <v-select id="branches" v-model="office.branches" :options="branches" label="name" placeholder="Search">
                    ...
                    <b-col md="6">
                        <!-- users -->
                        <b-form-group label="Users" class="my-custom-select">
                            <v-select v-model="office.users" placeholder="Search" :options="users" label="name">
                ...
                <div class="text-right mt-1">
                    <b-button size="sm" variant="primary" @click="() => {
                        ...
                        $refs['office-timings-modal'].hide()
                        resetform()
                    }" class="mr-1">
                        Cancel
                    </b-button>
        ...
        <div v-if="isLoading" class="b-overlay position-absolute"
          ...
          <div class="position-absolute d-flex align-items-center flex-column" style="top: 50%; left: 50%; transform: translateX(-50%) translateY(-50%);">
            ...
            <br><span>Adding Office Timings..</span>
          </div>
        </div>
    </b-modal>
</template>
    ...
    //   this.isValid = regex.test(ev)
        ...
        createTimings() {
            ...
            const data = {
                ...
                user_id:this.office.users != null ? this.office.users.id : null,
                branch_id:this.office.branches.id,
                office_days:count,
                officeDayTime:arr
            }
            this.$store.dispatch('officeTimings/store',data).then(() => {
                this.$refs['office-timings-modal'].hide()
                this.resetform()
            })
            console.log(data);
        }
