import officeTimings from '@/api/officeTimings'
...
const actions = {
  ...
  get({ commit }, id) {
    ...
    commit('SET_IS_LOADING', true);
    return new Promise((resolve, reject) => {
      officeTimings.get(id).then(({ data }) => {
        if(data.status == true){
          commit('SET_SUCCESS_MESSAGE', data.message);
          commit('SET_ALL', [data.data])
          resolve()
  ...
  store({ commit }, payload) {
    ...
    return new Promise((resolve, reject) => {
      officeTimings.store(payload).then(({ data }) => {
        if(data.status == true){
          commit('SET_SUCCESS_MESSAGE', data.message);
  ...
  update({ commit }, {payload, id}) {
    ...
    commit('SET_SIDEBAR_LOADING', true);
    return new Promise((resolve, reject) => {
      officeTimings.update({payload, id}).then(({ data }) => {
        if(data.status == true){
          commit('SET_SUCCESS_MESSAGE', data.message);
  ...
  destroy({ commit }, id) {
    ...
    return new Promise((resolve, reject) => {
      officeTimings.destroy(id).then(({ data }) => {
        if(data.status == true){
          commit('SET_SUCCESS_MESSAGE', data.message);
          resolve()
        } else {
