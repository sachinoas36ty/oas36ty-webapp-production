import subtaskTemplate from '@/api/subtaskTemplate'
...
const actions = {
  fetch({ commit }) {
    ...
    return new Promise((resolve, reject) => {
      subtaskTemplate.fetch().then(({ data }) => {
        if(data.status == true){
  ...
  store({ commit }, payload) {
    ...
    return new Promise((resolve, reject) => {
        subtaskTemplate.store(payload).then(({ data }) => {
        if(data.status == true){
          commit('SET_SUCCESS_MESSAGE', data.message);
          resolve()
        } else {
  ...
  update({ commit }, {payload, id}) {
    ...
    commit('SET_SIDEBAR_LOADING', true);
    return new Promise((resolve, reject) => {
        subtaskTemplate.update({payload, id}).then(({ data }) => {
        if(data.status == true){
          commit('SET_SUCCESS_MESSAGE', data.message);
          resolve()
        } else {
  ...
  destroy({ commit }, id) {
    ...
    return new Promise((resolve, reject) => {
        subtaskTemplate.destroy(id).then(({ data }) => {
        if(data.status == true){
          commit('SET_SUCCESS_MESSAGE', data.message);
          resolve()
        } else {
