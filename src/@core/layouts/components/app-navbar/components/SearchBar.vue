<template>
  <li class="nav-item nav-search">
    ...
    >
      <div class="search-input-icon" style="margin-top: -4px;">
      ...
      <b-form-input 
      class="p-1 pl-4 w-100 hello my-custom-select"      
        v-if="showSearchBar"
        v-model="searchQuery"
        placeholder="Search"
      ...
      >
        ...
        >
          ...
          <!-- Group Header -->
          <p class="suggestion-group-title border-l">
            <span style="font-weight: bolder;">
              {{ title(grp_name) }} 
            </span>
            <span style="color:grey">

              ({{ suggestion_list.length }} Result(s) Found)
            </span>
          </p>
          ...
          <ul>
            ...
            >
              ...
              <template v-if="grp_name === 'Clients'">
              ...
              <template v-else-if="grp_name === 'Users'">
                <div class="d-flex align-items-center">
                  <!-- <b-avatar
              ...
              <template v-else-if="grp_name === 'Tasks'">
                <div class="d-flex align-items-center">
                  <!-- <b-avatar
                    :src="suggestion.avatar"
                    class="mr-1"
              ...
              <template v-else-if="grp_name === 'Leads'">
                <div class="d-flex align-items-center">
                  <!-- <b-avatar
                    :src="suggestion.avatar"
                    class="mr-1"
...
export default {
  ...
  data(){
    return {
      ...
      suggestion:this.filteredData
    }
  },
  beforeUpdate() {
    this.searchAndBookmarkData.Clients.data = this.$store.state.clients.all
      this.searchAndBookmarkData.Users.data = this.$store.state.users.all
      this.searchAndBookmarkData.contacts.data = this.$store.state.contactPeople.all.map(e => e.data)
      this.searchAndBookmarkData.Tasks.data = this.$store.state.tasks.all.filter(e => e.type === 'task')
      this.searchAndBookmarkData.Leads.data = this.$store.state.leads.all.filter(e => e.type === 'lead')
  },
  setup() {
