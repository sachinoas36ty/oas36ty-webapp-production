  ...
  <div class="navbar-container d-flex content align-items-center ">
    ...
    border-bottom: 1px solid;">
                      <div class="demo-checkbox">
                        <input @click="updateTodo()" v-model="data.checked" type="checkbox" name="" id="" style="vertical-align: middle;">
                        <span class="break-word pre-formatted text-muted">
                            {{ data.to_do }}
                      ...
                      <span style="margin-top:1rem;font-size: 11px;
    color: #aaa;
    position: relative;
    bottom: -3px;
    ...
    right: 10px;" class="ownership">{{getUserName(data.user_id)}} - {{getDate(data)}}</span>
            ...
            <div class="text-right" >
              <button  :disabled="$store.state.appTodo.page_info.prev_page_url === null" class="text-right   btn btn-primary btn-sm mt-1 ml-1" @click="getPreviousTodos()">Previous</button>
            <button :disabled="$store.state.appTodo.page_info.next_page_url === null" class="text-right btn btn-primary btn-sm mt-1 ml-1" @click="getNextTodos()">Next</button>
          </div>
          </div>
